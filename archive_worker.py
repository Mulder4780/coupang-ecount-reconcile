...
DB_ONLY_ARCHIVE_FIELDS = frozenset(
    {
        ...
        "청구제외",
        "청구제외사유",
        # ★ 돌발AS 대응등급 A/B/C — 02시트에 대응 열이 **없다**(v632 실측 45열).
        #   `긴급도` 열이 있긴 하나 낱말이 `보통`·`높음` 이라 A/B/C 와 다르고
        #   646건 중 570건이 빈칸이라 사실상 죽어 있다 — 그 칸에 밀어 넣으면
        #   원장 의미가 깨진다([172]). 위 갈래들과 같은 자리다.
        #   ⚠ 나중에 02시트에 열이 생기면 **여기서 빼기만 하면** 저절로 엑셀에 실린다
        #     (2026-09-09 형님 지시 "앱 DB에만 담고 나중에 엑셀에 담아").
        "대응등급",
        "대응유형",
        "등급사유",
    }
        ...
        store.update_work(
            ...
            patch={
                "fields": {
                    ...
                    "청구제외": "예",
                    "청구제외사유": "캠프 부담 — 합성",
                    # 돌발AS 대응등급 A/B/C(2026-09-09 지시). 위 assert 가 표 전부를
                    # 여기서 쓰라고 요구하므로 이 셋도 한 번은 지나가야 한다.
                    # ★ 낱말은 as_grade 가 정하는 그 값을 그대로 쓴다([162]) —
                    #   합성이라도 진짜 모양이어야 이 길이 진짜를 잰다.
                    "대응등급": "A긴급",
                    "대응유형": "즉시출동",
                    "등급사유": "리프트 작동불능 — 합성",
                }
