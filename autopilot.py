...
#    · `verification_sync.py` — 같은 모양이고 자리만 다르다(2026-09-06 실측).
#      일을 시작하기 **전에** 근거부터 모으는데 그것만 **2,455초(40.9분)** 다:
#      관리대장 찾기 11.8 + 워크북 열기 3.8 + 밴드 근거 108.8 +
#      **ERP 근거 2,312.9** + 명세서 근거 17.4. 09:50 회차의 그 단계는 600초
#      제한이라, 예산을 주면 근거를 모으다 다 되어 **한 건도 못 하고 75 로**
#      돌아오는 것을 되풀이한다. 고칠 자리는 예산이 아니라 그 2,313초다
#      (분담판 [385] - 캐시가 왜 매번 무효인지부터 잰다).
...
RESOURCE_MARKERS = (
    ...
    "폴더에 닿지 못했습니다",
    "정기점검 원본 폴더가 없습니다",
    # ★ 저쪽(이카운트 서버)이 지금 안 받는 것도 **우리 코드 고장이 아니다**.
    #   낱말은 만드는 쪽(`erp_api_collect._SIGNS`)이 쓰는 **그 글자**다 —
    #   어긋나면 한 건도 안 걸리면서 오류도 안 난다([165]).
    #   ★ **넓히지 않는다**([172]): 맨몸 `503` 을 쓰면 남의 출력에도 흔해
    #     진짜 코드 고장까지 '자원 탓'이 되어 아무도 안 고친다.
    "저쪽 서버가 안 받음",
    "winerror 53",
    "z:\\",
    "z:/",
...
def stuck(doc: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for x in items:
        ...
        #   늘어나면 옛 티켓은 저장된 `code` 를 그대로 들고 있어, 새 이름이
        #   **다음 재시도(최대 8시간) 전까지 안 붙는다** — 그동안 조치가 사람을
        #   멀쩡한 코드로 보낸다([172]).  그래서 `code` 일 때만 다시 재 본다.
        # ★ **`config` 로 갈 때만 받는다**([172]).  `code` 를 통째로 다시 재면
        #   `resource`·`timeout` 까지 바뀌어 **갈래를 넘어 완화한다** — 실측으로
        #   `t424`(자원 실패가 이미 풀렸나)가 그 자리에서 깨졌다.  여기서 하려는
        #   것은 **새로 만든 이름을 옛 티켓에도 붙이는 것**이지 저장된 갈래를
        #   다시 매기는 것이 아니다.
        if 갈래 == "code" and raw and classify_failure(raw) == "config":
            갈래 = "config"
