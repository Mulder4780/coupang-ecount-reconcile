...
def report(g, tbl, since, until):
    ...
    if g["전표만"]:
        ...
        ahead = [x for x in g["전표만"] if x["일자"] > today]
        now_ = [x for x in g["전표만"] if x["일자"] <= today]
        L += ["## ③ 매출전표는 있는데 계산서가 안 보인다 (%d건)" % len(g["전표만"]), "",
              "> **선일자 전표 %d건은 정상이다** — 아직 오지 않은 날짜라 계산서가"
              " 안 나가는 것이 맞다. 실제로 볼 것은 아래 %d건이다."
              % (len(ahead), len(now_)), "",
              "| 일자 | 전표번호 | 거래처 | 금액 | 적요 | |",
              "|---|---|---|---:|---|---|"]
        for x in now_[:200] + ahead[:200]:
            L.append("| %s | %s | %s | %s | %s | %s |"
                     % (x["일자"], x["전표번호"], x["거래처"], WON(x["금액"]),
                        str(x["적요"])[:36], "선일자" if x["일자"] > today else "**확인**"))
        L.append("")
    if g["금액다름"]:
