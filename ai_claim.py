...
# 잡을 수 있는 것들. ledger 는 배타적, 나머지는 겹쳐도 된다.
LOCKS = {
    "ledger":  ("관리대장 쓰기", True),
    "band":    ("밴드 수집·반영", True),
...
def load():
    try:
        ...
        d = {}
    now = time.time()
    # 오래된 점유는 자동 해제 — 크레딧이 끊긴 AI가 영원히 붙들고 있으면 안 된다
    return {k: v for k, v in d.items() if now - v.get("at", 0) < STALE}


def save(d):
    os.makedirs(os.path.dirname(CLAIMS), exist_ok=True)
    json.dump(d, open(CLAIMS, "w", encoding="utf-8"), ensure_ascii=False, indent=1)
...
def take(who, what, why=""):
    ...
    if cur and cur.get("who") != who and excl:
        ...
        return False
    d[what] = {"who": who, "why": why, "at": time.time(),
