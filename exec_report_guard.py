...
#: 합성 데모가 섞였는지는 **낱말이 아니라 상수**로 본다.
#:   첫 판이 `합성`·`데모`·`예시` 를 훑었다가 실제 행(`UJ2600201`)을 지목했다 —
#:   그 낱말들은 사람이 적은 문제 본문에도 나온다.  잘못 지목하면 **멀쩡한 값을
#:   고치러 간다**(`[172]`).  그래서 `app_server` 의 `if DEMO:` 분기가 실제로 만드는
#:   식별자만 본다 — 실데이터에는 있을 수 없는 값들이다.
DEMO_IDS = ("UJ261001", "JS-2607-001")
DEMO_CAMP_PREFIX = "합성 캠프"
...
def build(day: Optional[str] = None) -> Dict[str, Any]:
    ...
    for label, col in MONEY_COLUMNS.items():
        ...
        if health["채워짐"] == 0:
            out["먼저볼것"].append(
                "**%s = %s** 인데 근거 열 `%s` 은 **한 행도 안 채워져 있다**(%d행 중 0). "
                "이 숫자는 '0원'이 아니라 **'못 셈'** 이다 — 화면이 확언하면 안 된다"
                % (label, amount, col, health["총행"]))
        elif health["값있음"] == 0:
            out["먼저볼것"].append(
                "**%s** 의 근거 열 `%s` 은 채워지긴 했으나 **값이 있는 행이 0개**다"
                % (label, col))
        elif health["값비율"] < MOSTLY_ZERO_RATIO:
            out["먼저볼것"].append(
                "**%s = %s (%s건)** — 근거 열 `%s` 은 %d행 중 **값 있는 행이 %d개뿐**이다"
                "(%.1f%%). 이 합계는 '잔액'이 아니라 '누가 한 칸 적었다'는 뜻일 수 있다"
                % (label, amount, count, col, health["총행"], health["값있음"],
                   health["값비율"] * 100))

    # ③ 합성 데모가 섞였나 — **식별자로만** 본다(낱말로 훑으면 실제 행을 지목한다)
    for label, d in details.items():
        for r in (d.get("rows") or [])[:400]:
            pid = str(r.get("프로젝트NO") or "").strip()
            rid = str(r.get("ID") or "").strip()
            camp = str(r.get("캠프명") or "")
            if pid in DEMO_IDS or rid in DEMO_IDS or camp.startswith(DEMO_CAMP_PREFIX):
                out["먼저볼것"].append(
                    "**%s** 에 합성 데모 행이 섞였다(%s) — 캡처로 내보내면 지어낸 숫자가 나간다"
                    % (label, pid or rid or camp))
                break
