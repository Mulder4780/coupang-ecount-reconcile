...
# ★ 2026-07-29 실사고: 앞의 규칙은 `▒` 에서 값을 끊었다. 그런데 'A/S 내용' 의 값은
#   `▒▒ 01 호기 ▒▒ (유료)릴레이 2EA 납품…` 처럼 **정확히 `▒▒` 로 시작한다.**
#   그래서 작업 내용이 **언제나 빈 값**이었고, 03시트 실적 칸이 영영 안 채워졌다.
#   값의 끝은 다음 항목 표시(`●`)나 안내문(`★`)·원인조치(`♠`)·확인사항(`※`)으로 본다.
RE_FIELD = re.compile(r"[●∙•]\s*([^:：●]{1,20}?)\s*[:：]\s*(.*?)(?=\s*[●∙•]|\s*★|\s*♠|\s*※|$)")
...
# 공지 서식의 **빈 양식** — 기사가 아직 안 채운 칸이다.
RE_BLANK_FORM = re.compile(r"\?\?\s*호기|What\s*\?|갯수\s*\?")


def work_content(v):
    """'A/S 내용' 에서 **실제로 적힌 것**만 값으로 인정한다.

    ★ 2026-07-29 실사고: 같은 프로젝트가 '안내' 와 '완료' 로 두 번 올라오는데,
      안내 글의 A/S 내용은 `▒▒ ?? 호기 ▒▒ (유료)What ? / 갯수 ?` 라는 **빈 양식**이다.
      이걸 값으로 치면 나중 완료 글의 진짜 내용(예: '릴레이 2EA 납품')이 병합에서 밀려나
      실적 칸이 영영 비어 있게 된다. 실제로 UJ2601350 등이 그랬다.
    → 빈 양식 조각을 걷어내고 남는 게 없으면 **빈 값으로 돌려준다.**"""
    t = clean(v)
    if not t:
        return ""
    core = RE_BLANK_FORM.sub(" ", t)
    core = re.sub(r"[▒()/]|유료|무료|작업자|공임|호기|[\d\s.·-]", "", core)
    return "" if len(core) < 2 else t
...
def _stage(kind, _memo={}):
    """새 건이 처음 갖는 단계 낱말 — **정본에서 읽는다. 코드에 적지 않는다.**

    ★ 왜 함수인가 (분담판 [48] · 2026-08-10 실측). 앱이 코드에 박힌 `"접수"` 를
      넣고 있었는데 그 낱말은 관리대장 드롭다운에 **없다**(목록은 `신규접수`).
      원장 85건이 그 값이라 엑셀에서 그 칸을 열면 사람이 다시 골라야 했는데,
      화면은 멀쩡히 '접수'라고 보여 주므로 **아무도 몰랐다**(검증 [166]).
      앱 경로는 그날 고쳐졌는데 **카톡 경로만 안 따라왔다** — 여기가 그 자리다.
    ★ 못 읽으면 **비워 둔다.** 지어낸 낱말을 넣는 것이 빈칸보다 나쁘다.
    ★ 한 번만 읽는다 — `default_stage` 는 워크북을 여는 길이라 행마다 부르면 비싸다.
    """
    if kind not in _memo:
        try:
            import work_flow
            _memo[kind] = work_flow.default_stage(kind) or ""
        except Exception:
            _memo[kind] = ""
    return _memo[kind]
...
# ⚠ '진행상태'·'점검상태' 는 수식 열이 아니라 **사람이 고르는 드롭다운**이다 —
#   제외될 이유가 없었는데 빠져 있어서, 카톡으로 들어온 건이 전부 빈칸이었다.
#   안 쓰는 열은 빈칸과 구별할 수 없다(검증 [165]). `only_if_empty` 로 넣으므로
#   사람이 이미 고른 값은 덮지 않는다.
SHEET_MAP = {
    "02_돌발AS접수": {
    ...
    # 철거·보관 / 납품설치 — AS·점검이 아닌 **별도 공사**다.
    # 05시트에는 '철거·이전예정일' 열이 따로 있고 기존 행도 전부 '철거' 라 여기가 자리다.
    # ★ 이 시트는 앱이 읽지 않는다 — 사용자 지시(2026-07-29) "DB만 저장하고 앱에 표기하지마"
    #   와 정확히 맞는다. 앱에 띄우고 싶어지면 시트를 옮기지 말고 앱 쪽 필터를 풀 것.
    "05_신규납품설치": {
        "프로젝트NO": "프로젝트NO", "캠프명": "캠프명", "업무구분": "_업무구분",
        "요청일": "신청일자", "담당자": "담당기사", "비고": "_비고",
        "철거·이전예정일": "_철거예정", "납품예정일": "_납품예정",
    },
}
# kind_of() 가 내는 유형 → 05시트 업무구분(10_코드관리 M열 허용값 안에서만 쓴다)
SIDE_KIND = {"(철거·보관)": "철거", "(납품설치)": "납품"}
...
def build_queue(rows, master):
    ...
    for r in rows:
        # 철거·납품은 05시트 한 곳으로 모은다(업무구분으로 갈린다)
        if r["시트"] in SIDE_KIND:
            r = dict(r, _업무구분=SIDE_KIND[r["시트"]], 시트="05_신규납품설치")
            r["_철거예정"] = r["예정일"] if r["_업무구분"] == "철거" else None
            r["_납품예정"] = r["예정일"] if r["_업무구분"] == "납품" else None
    ...
    for sheet, recs in groups.items():
        ...
        for r in recs:
            ...
            src = dict(r, _비고=note[:250], _경로="카톡",
                       ...
                       _진행상태=_stage("as"), _점검상태=_stage("pm"))
