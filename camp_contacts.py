...
#: 캠프명 <-> ERP 거래처코드 대조표(`camp_code_match.py` 산출물).
#: ★ 09:50 회차에서 **캠프 담당자 뒤**에 만들어지므로 여기서 읽는 것은
#:   한 회차 전 것이다 — 첫 회차에는 안 붙고 다음 회차부터 붙는다(수렴).
CODEMAP = os.path.join(REPORT_DIR, "캠프_거래처코드_대조.json")
...
#: 대조표가 만드는 갈래(A~H) → 캠프 행에 적을 낱말과 그 뜻.
#: ★ **갈래를 새로 만들지 않는다**([162]) — `camp_code_match` 가 정한 것을 그대로
#:   빌려 **사람 말**만 붙인다. 여기서 다시 판정하면 같은 캠프를 두 화면이
#:   서로 다른 코드로 부르고, 갈린 뒤에는 어느 쪽이 맞는지 아무도 모른다.
CODE_STATE = {
    "A": ("확정", "ERP 거래처명과 정확히 같다"),
    "C": ("확정", "ERP 등록값 그대로다 — 다만 쿠팡 CU### 형식이 아니다"),
    "G": ("추정", "이름·주소로 후보가 하나뿐이라 지목한 것 — 확정이 아니다"),
    "D": ("후보여럿", "후보가 여럿이다 — 사람이 고른다"),
    "H": ("후보여럿", "후보가 여럿이거나 지역 표기가 어긋난다 — 사람이 고른다"),
    "E": ("없음", "ERP 거래처등록에 댈 후보가 없다 — 매니저 확인이 필요하다"),
    "F": ("코드빔", "ERP·원장에 있는데 거래처코드 칸이 비어 있다"),
}


def apply_codes(rows, path=None):
    """캠프 행에 **ERP 거래처코드**를 붙인다 — 판정은 대조표 것을 빌린다([162]).

    ★ 왜 필요한가 (노승용 매니저 2026-08-25 카톡): 카톡·밴드 접수 글의 캠프 정보가
      ERP 등록과 달라 유현민·전상희 매니저가 **하나하나 조회해 매칭**하고 있다.
      "거래처코드+거래처명만 맞아도 관리가 200,000% 쉬워진다."
      그런데 실측 2026-08-25 — `캠프_담당자.json` 739행의 `거래처코드` 칸이
      **있는데 값은 0건**이었다(칸이 있어 화면은 멀쩡해 보인다 · [165] 의 그 모양).
      뿌리는 `캠프마스터.json` 이 그날 ERP 를 **한 건도 못 읽은 것**이다
      (198행 출처가 전부 `원장+밴드`). 그런데 **대조표는 같은 날 답을 갖고 있었다** —
      확정 175(A 174·C 1) · 추정 182(G). 곧 **대조표와 캠프 화면이 안 이어져 있는
      것**이 전부이고, 새로 계산할 것이 없다.

    ★ **확정만 `거래처코드` 칸에 넣는다 — 이것이 이 함수에서 제일 중요한 문이다.**
      대조표는 다음 회차에 **이 파일의 `거래처코드` 를 확정으로 읽는다**
      (`camp_code_match.build()` ② 밴드 갈래). 그러니 추정을 그 칸에 넣으면
      **다음 회차에 추정이 확정으로 굳는다** — 되돌릴 수 없는 쪽이다([172]).
      추정은 `추정코드` 라는 **다른 칸**에 담는다(대조표는 그 칸을 안 읽는다).

    ★ **후보가 유일할 때만 붙인다**([172]) — 캠프명과 `다른표기` 로 각각 찾아
      **서로 다른 코드**가 나오면 그것은 '둘 다'가 아니라 **'모른다'** 이다.
      틀린 코드가 유니웍스에 박히면 빈 칸보다 나쁘다(이 표를 읽는 사람이
      그것을 그대로 적는다).

    ★ **이미 있는 값을 덮지 않는다.** 다르면 조용히 무시하지도 않고 `충돌` 로
      적는다 — 어느 쪽이 맞는지는 사람이 안다([169]).

    ★ **못 읽으면 '없다'가 아니라 '못읽음'** 이다([169]). 대조표가 아직 안 만들어진
      첫 회차에는 코드가 안 붙고 다음 회차부터 붙는다(수렴한다).
    """
    p = path or CODEMAP
    try:
        with open(p, encoding="utf-8") as f:
            표 = json.load(f)
        대조 = 표.get("rows") or []
        if not isinstance(대조, list):
            raise ValueError("rows 가 목록이 아니다")
    except Exception as e:                       # noqa: BLE001
        for r in rows:
            r.setdefault("추정코드", "")
            r["코드갈래"] = "못읽음"
...
#: 원본 양식이 적어 보낸 코드로 채웠을 때 근거 칸에 남기는 말머리.
#  * **쓰는 쪽과 세는 쪽이 같은 글자를 봐야 한다**([165]) - 손으로 두 번 적으면
#    문구를 고치는 날 세는 쪽이 조용히 0이 되면서 오류도 안 난다.
POSTED_CODE_NOTE = "접수 양식에 적혀 온 값"
#  * 충돌 표시는 채움 표시(POSTED_CODE_NOTE)의 **앞부분이면 안 된다** - 처음에
#    "접수 양식에 적혀 온" 으로 뒀더니 채운 3건이 충돌로도 세였다(실측 채움 3
#    = 충돌 3 이 그 증거다 - [165] 의 그 모양). 겹치지 않는 말로 둔다.
POSTED_CODE_CLASH = "접수 양식 코드와 어긋난다"

def posted_codes(recs):
    """접수 양식에 **원본이 직접 적어 보낸** ERP 거래처코드를 캠프별로 모은다.

    2026-09-02 형님 지시: *"거래처 코드는 최신 카톡 자료 및 최신 자료가 맞는거야
    그걸로 업데이트해 계속 들어가는 자료 참고해서 업데이트하는 알고리즘 구현해"*

    * **왜 이것이 세나** — 대조표(`camp_code_match`)는 이름/주소가 닮은 것을 보고
      **추정**한다. 이것은 접수하는 사람이 **그 캠프 앞으로 그 순간 적은 값**이라
      근거가 다르다. [326] 이 정기점검 스케줄 원본을 밴드보다 세게 본 것과 같다.
    * **최신이 이긴다**(형님 지시) — 같은 캠프에 코드가 여러 번 적혔으면 가장 나중
      글의 값을 쓴다. 캠프가 옮기거나 ERP 등록이 바뀔 수 있다.
    * 값이 여럿이면 **그 사실을 적는다**([169]) — 조용히 최신만 남기면 사람이
      "왜 이 코드지"를 물을 근거가 없다.  실측 2026-09-02 에는 0건이다.
    * **캐시를 안 만든다**([168]) — 부르는 쪽(`build`)이 이미 `load_records()` 로
      읽어 둔 것을 넘겨받아 훑기만 한다.
    """
    got, when, seen = {}, {}, {}
    for r in recs or []:
        code = str(r.get("거래처코드") or "").strip().upper()
        camp = str(r.get("캠프명") or "").strip()
        if not code or not camp:
            continue
        key = _norm(camp)
        if not key:
            continue
        day = str(r.get("게시일") or "")
        seen.setdefault(key, set()).add(code)
        # 최신 글이 이긴다 - 날짜가 같거나 비면 먼저 본 것을 그대로 둔다.
        if key not in got or day > when.get(key, ""):
            got[key], when[key] = code, day
    여럿 = {k: sorted(v) for k, v in seen.items() if len(v) > 1}
    return got, when, 여럿


def apply_posted_codes(rows, recs):
    """원본이 적어 보낸 코드로 캠프 행을 갱신한다 - `apply_codes` **뒤에** 얹는다.

    * **덮지 않는다**([172]) — 이미 확정 코드가 있는데 다르면 `충돌` 로 남기고
      사람이 고른다.  틀린 코드가 유니웍스에 박히면 빈 칸보다 나쁘다.
    * **같은 값이면 한 글자도 안 바꾼다**([235]-4 와 같은 판단) — *"ERP 거래처명과
      정확히 같다"* 는 기계가 확인할 수 있는 근거라 이것보다 세다.  덮으면 그
      근거를 잃는다.  실측 2026-09-02: 22곳 중 **20곳이 이미 같음**이다.
    * **빈 칸이거나 추정뿐일 때만 채운다** — 그때가 이 알고리즘이 값을 하는 자리다
      (실측 3곳: 김포1Sub-FC - 전주1Sub-hub - 호성MB).
    * 확정이 서면 **추정코드는 지운다** — 한 줄에 두 코드가 있으면 사람이 못 고른다
      (`apply_codes` 가 이미 지키는 규칙 그대로 · [162]).
    * **못 읽으면 아무것도 안 한다**([169]) — 모름을 근거로 코드를 지우거나
      바꾸지 않는다.
    """
    got, when, 여럿 = posted_codes(recs)
    if not got:
        return {"원본기재": 0, "원본채움": 0, "원본충돌": 0, "원본코드여럿": 여럿}
    채움 = 충돌 = 같음 = 0
    for r in rows:
        code = ""
        for name in [r.get("캠프명")] + list(r.get("다른표기") or []):
        ...
        if 기존:
            ...
            충돌 += 1
            r["코드갈래"] = "충돌"
            r["코드근거"] = ("%s - 이미 적힌 %s, 양식은 %s" % (POSTED_CODE_CLASH, 기존, code)
                             + " (양식 게시일 %s)" % (day or "모름"))
            continue
        채움 += 1
        r["거래처코드"] = code
        r["추정코드"] = ""
        r["코드갈래"] = "확정"
        r["코드근거"] = ("%s (게시일 %s)" % (POSTED_CODE_NOTE, day or "모름"))
    return {"원본기재": 같음 + 채움, "원본채움": 채움, "원본충돌": 충돌,
            "원본코드여럿": 여럿}
...
def build():
    ...
    # ★ **맨 마지막에** 붙인다 — 정본(`apply_pm_schedule`)이 캠프명을 바꿀 수 있어
    #   그 뒤 이름으로 대조표를 찾아야 짝이 맞는다.
    코드 = apply_codes(rows)
    # * **원본이 적어 보낸 코드가 대조표 추정보다 세다**(2026-09-02 형님 지시).
    #   그래서 apply_codes **뒤에** 얹는다 - 순서를 뒤집으면 약한 근거(추정)가
    #   먼저 자리를 잡아 원본이 못 들어간다([326] 과 같은 규칙).
    원본코드 = apply_posted_codes(rows, recs)
    ...
    return {
        ...
        "정기점검원본": sched_왜,
        "정본반영": 정본,
        # ★ 붙인 것을 **숫자로 말한다**([169]) — 조용히 0건이면 아무도 안 본다.
        **코드,
        "rows": rows,
        **보완,
        ...
        # * 행에서 못 세는 둘은 여기서 싣는다([169]) - "이미 같음"은 근거 칸을
        #   안 바꾸므로(같은 값이면 한 글자도 안 건드린다) 행만 봐서는 알 수 없다.
        "원본기재": 원본코드.get("원본기재", 0),
        "원본코드여럿": 원본코드.get("원본코드여럿") or {},
        **summarize(rows),
    }
...
def summarize(rows):
    ...
    return {
        ...
        "코드붙임": 갈래,
        "코드충돌": 갈래.get("충돌", 0),
        # * **조용히 바꾸지 않는다**([169]) - 원본 양식이 적어 보낸 코드로 몇 곳을
        #   채웠는지/몇 곳이 이미 같았는지/충돌이 몇인지를 숫자로 말한다.
        # * **행에서 센다**([235]-4) - `apply_posted_codes` 가 제 손으로 세면 그
        #   숫자는 회차가 만든 그 순간의 것이라, 사람이 코드를 확인해 주거나
        #   캠프를 숨겨도 머리글이 안 따라간다([169]).
        "원본채움": sum(1 for r in rows
                        if str(r.get("코드근거") or "").startswith(POSTED_CODE_NOTE)),
        "원본충돌": sum(1 for r in rows
                        if POSTED_CODE_CLASH in str(r.get("코드근거") or "")),
    }
