...
def traces():
    ...
    for path in sorted(_glob.glob(os.path.join(ROOT, "reports", "*_오류.json"))):
        ...
        out.append({"파일": os.path.basename(path), "시각": d.get("시각", ""),
                    ...
                    "작업": str(d.get("작업") or ""),
                    # ★ **이름이 둘이라 조치가 인계에 안 가고 있었다** (2026-08-28 실사고).
                    #   자국을 남기는 쪽이 두 낱말로 갈려 있다 — `어떻게` (browser_chain·
                    #   recollect·coordinate·schedule_watch) 와 **`조치`** (daily_run·
                    #   noon_run·camp_contacts·erp_api_collect). 그런데 여기서는
                    #   `어떻게` 만 물어서, `조치` 로 적는 회차의 조치는 **한 번도
                    #   안 실렸다**([165] — 어긋나면 한 건도 안 걸리면서 오류도 안 난다).
                    #   실측 2026-08-28 09:50: `_gate_fix` 가 *"t49 가 막았다 — 그
                    #   검증부터 본다"* 를 정확히 만들어 뒀는데 인계에 나간 조치는
                    #   `type reports\일일대조_오류.json`(= 파일을 열어 보라)뿐이었다.
                    # ★ **쓰는 쪽을 통일하지 않는다**([247]) — 이미 디스크에 적힌 자국이
                    #   있고, 쓰는 쪽만 맞춰 봐야 **옛 자국은 여전히 안 읽힌다.**
                    #   읽는 쪽이 두 이름을 다 받는다([212] 가 시각 모양에서 배운 그대로).
                    "어떻게": str(d.get("어떻게") or d.get("조치") or "")[:160]})
    return out
