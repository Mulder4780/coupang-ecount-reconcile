...
def _양보상대(칸, 연속):
    """연속 양보한 **상대**를 흔한 순으로 요약한다.

    ★ 왜 필요한가(2026-09-06 실측): 굶주림 문구가 *"수집이 6회 연속 양보했다"*
      까지만 말해 **누구에게** 가 없었다.  그런데 자국에는 `주인` 이 그대로 있고
      6번이 **전부 `일일대조`** 였다 — 곧 고칠 자리는 '수집'이 아니라 **그 회차가
      Z: 를 오래 무는 것**이다.  아는 것을 안 적으면 사람이 엉뚱한 데를 본다([169]).
    ★ **지어내지 않는다**([169]) — `주인` 칸이 비면 빈 문자열이고, 그때는 문구가
      예전 그대로다.  모르는 것을 아는 것처럼 적지 않는다.
    """
    from collections import Counter
    xs = [x for x in 칸 if isinstance(x, dict)]
    최근 = xs[-연속:] if 연속 > 0 else []
    이름 = [str(x.get("주인") or "").strip() for x in 최근
            if x.get("갈래") == "양보"]
    이름 = [n for n in 이름 if n]
    if not 이름:
        return ""
    c = Counter(이름)
    if len(c) == 1:
        return "전부 **%s** 에게" % 이름[0]
    return " · ".join("%s %d회" % (n, k) for n, k in c.most_common())
...
def notices(d=None, why="", now=None, 도는중=None):
    ...
    for 작업 in sorted(d):
        ...
        if h["연속양보"] < STARVE_N:
            continue
        # 2026-09-05 실측 - 막혔다는 말은 "두드렸는데 안 열렸다"는 뜻이다. 사람이 손으로
        #   부르는 문(`수집` - collect_gate)은 안 부르면 자국 자체가 안 생기는데, 옛 판정은
        #   마지막 **완주**만 보고 굶주림이라 했다 - 09-04 15:44 에 셋째 양보를 남긴 뒤
        #   **31시간 동안 자국이 0건**인데도 매일 인계 맨 위에 떴다. 거짓 경보는 진짜를
        #   덮는다(`[170]`). 좁히는 것도 고장이라(`[172]`) 지금도 양보가 이어지는 회차
        #   (정오회차 오늘 12:50)는 그대로 잡힌다. 값은 STARVE_HOURS 를 그대로 쓴다(`[162]`).
        #   조용히 빼는 것이 아니다(`[169]`) - 아래 `_md` 가 작업마다 history 를 돌며 적는다.
        최근양보 = _때(h["마지막양보"] or {})
        if not 최근양보 or now - 최근양보 > timedelta(hours=STARVE_HOURS):
            continue
        마지막 = _때(h["마지막돎"] or {})
        # ★ **매일 양보만 하는 회차는 없는 회차와 같다** (noon_run 이 제 안에서 배운 것을
        #   모든 작업에 넓힌다). 양보는 실패가 아니라서 아무 경보도 안 뜨는 자리다.
        누구 = _양보상대(d.get(작업) or [], h["연속양보"])
        꼬리 = ("(%s)" % 누구) if 누구 else ""
        if h["한번도안돎"]:
            out.append({"갈래": "굶주림", "작업": 작업,
                        "무엇": "**%s** 이 %d회 연속 양보만 했고%s **한 번도 돈 적이 없다** — "
                                "겹치는 쪽을 옮기거나 이 작업이 필요한지 다시 본다"
                                % (작업, h["연속양보"], 꼬리),
                        "어떻게": "python coordinate.py --print"})
        elif 마지막 and now - 마지막 > timedelta(hours=STARVE_HOURS):
            out.append({"갈래": "굶주림", "작업": 작업,
                        "무엇": "**%s** 이 %d회 연속 양보했고%s 마지막으로 돈 것은 %s 다"
                                % (작업, h["연속양보"], 꼬리,
...
# ── 리포트 ──────────────────────────────────────────────────────────────────
def _md(d, why, now=None, 도는중=None):
    now = now or datetime.now()
    L = ["# 겹치는 일 조율", "",
         "- 잰 때: %s · 작업 %d개" % (now.isoformat(timespec="seconds"), len(d)), ""]
    # 도는중 을 그대로 넘긴다 — 여기서 안 넘기면 아래 notices/audit 이 각자 락을
    # 다시 읽어, 같은 표를 놓고 부르는 순간마다 다른 답이 나온다.
    # 실측 2026-08-23: 관문(t293)이 daily_run 0단계 안에서 돌 때 그 회차가 제 락을
    # 쥐고 있어 판정이 미뤄졌고, 리포트에서 '뒤에 스스로 된 것' 칸이 통째로 사라져
    # 매일 아침 회차가 첫 줄에서 죽었다(손으로 돌리면 초록이다 · [235]).
    ns = notices(d, why, now=now, 도는중=도는중)
    if ns:
    ...
    try:
        _헛, _못, _점, _뒤 = audit(d, now=now, 도는중=도는중)
        if _점:
