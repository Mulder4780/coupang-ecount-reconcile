...
BOOK = [
    {
        # 2026-08-28: 형님 `CSOS-UPLOAD-400` 신고에서 시작해 서버의 거절 길을
        # 세었다. **막음을 안 단다**([292]) — 정상 거절이지 고친 고장이 아니다.
        # ★ 받는 형식 목록을 여기 적지 않는다([162]) — 화면마다 다르고
        #   (PO 11개 · 입금 7개 · 업무일지 `.xlsx` 하나뿐) 서버 문구가
        #   이미 제 목록을 말한다. 여기 적으면 사본이 되어 곧 틀린 안내가 된다.
        "이름": "이 화면이 안 받는 파일 형식입니다",
        "when": ["지원되지 않는 원본 파일 형식"],
        "쉬운말": "고르신 파일의 형식을 이 화면이 안 받습니다. "
                "오류 문구 괄호 안이 **고르신 형식**이고 그 뒤가 **이 화면이 받는 형식**입니다.",
        "왜": "화면마다 받는 형식이 다릅니다 — 업무일지는 엑셀 하나만 받고, "
              "입금은 엑셀·csv·사진·pdf 를 받고, 쿠팡 PO 는 txt·zip 까지 받습니다.",
        "하세요": ["오류 문구 뒤의 '받는 형식' 을 그대로 봅니다.",
                 "카카오톡 내보내기(.txt)면 이 화면이 아니라 **카톡 전용 올리기**를 씁니다.",
                 "표를 사진으로 찍으신 것이면 원본 엑셀·pdf 로 올리는 편이 정확합니다."],
    },
    {
        # ★ **틀린 지목을 막는 자리다**([172]) — 사람은 **경로를 붙여넣었는데**
        #   "찾지 못했습니다" 를 듣는다. 경로가 틀린 것이 아니라 그 안에
        #   **받는 형식의 파일이 없는** 것이다.
        "이름": "폴더 안에 받는 형식의 파일이 없습니다",
        "when": ["경로 안에서 지원되는 원본 파일을 찾지 못했습니다"],
        "쉬운말": "붙여넣으신 경로는 찾았습니다. 다만 그 안에 **이 화면이 받는 "
                "형식의 파일이 없습니다.** 경로를 잘못 넣으신 것이 아닙니다.",
        "왜": "폴더를 주시면 그 안을 훑어 받는 형식만 담습니다. 하나도 없으면 "
              "담을 것이 없어 거절합니다.",
        "하세요": ["폴더 대신 **파일을 곧바로** 골라 봅니다.",
                 "오류 문구 뒤의 '받는 형식' 과 그 폴더 안 파일을 대 봅니다.",
                 "임시 파일(`~$` 로 시작하는 것)은 세지 않습니다 — 엑셀을 닫고 다시 해 봅니다."],
    },
    {
        "이름": "파일이 너무 큽니다",
        "when": ["첨부파일은 55MB 이하만"],
        "쉬운말": "한 번에 올릴 수 있는 크기를 넘었습니다.",
        "왜": "큰 파일 하나가 올라가는 동안 다른 담당자 화면이 같이 느려집니다.",
        "하세요": ["엑셀이면 필요한 시트만 남겨 따로 저장해 올립니다.",
                 "사진이 여럿이면 나눠서 몇 번에 올립니다.",
                 "그래도 커야 하면 원본 폴더에 두고 **경로를 붙여넣습니다**."],
    },
    {
        "이름": "올릴 것을 아직 안 주셨습니다",
        "when": ["파일을 선택하거나 URL"],
        "쉬운말": "파일도 경로도 안 들어왔습니다.",
        "왜": "고르신 파일이 화면에 안 실렸거나, 붙여넣은 칸이 비어 있습니다.",
        "하세요": ["[파일 선택] 을 눌러 다시 고릅니다.",
                 "끌어다 놓으셨으면 상자 안에 파일 이름이 뜨는지 봅니다.",
                 "경로를 붙여넣으실 때는 **이 PC 의 원본자료·바탕화면·다운로드** 안이어야 합니다."],
    },
    {
        "이름": "저장 버전 없음",
    ...
    {
        # 2026-08-27 류지영 실사고: `SCH-2026Q3-25D510AD97` 일산2MB 카드에 담당기사·
        # 점검상태·예정일·실제점검일·비용구분 **다섯 칸을 다 채우고** 저장을 눌렀더니
        # "목록이 **반쪽으로** 온 것일 수 있습니다" 가 나왔다. 그 건은 반쪽이 아니라
        # **앱 DB 에 행이 처음부터 없는** 스케줄 원본 예정이다 — 새로고침해도 안 풀린다.
        # `[466]` 이 카드에서 저장 단추를 빼고 [+ 등록하고 완료 처리] 로 바꿨다.
        # ★ 이 항목이 **위**에 있어야 한다 — 아래 일반 항목이 먼저 걸리면 사람이
        #   "목록을 다시 받으세요" 를 아무리 해도 제자리다([292]·[172]).
        "이름": "아직 등록 안 된 예정입니다",
        "when": ["NO_VERSION:pm:SCH-", "NO_VERSION:as:SCH-"],
        "쉬운말": "이 건은 정기점검 스케줄 원본에서만 온 **예정**이라 앱에 아직 "
                "기록이 없습니다. 목록이 반쪽으로 온 것이 아닙니다.",
        "왜": "앱에 없는 기록은 고칠 자리도 없어 저장이 막힙니다. "
              "새로고침해도 같습니다 — 없는 것은 새로고침으로 안 생깁니다.",
        "하세요": ["카드의 [+ 등록하고 완료 처리] 를 누릅니다 "
                 "(적어 두신 값이 그대로 넘어갑니다).",
                 "폼에서 값을 확인하고 저장합니다.",
                 "그 뒤로는 보통 기록처럼 수정·저장이 됩니다."],
        "막음": "[466]", "고친날": "2026-08-27",
    },
    {
        # ⚠ 여기에는 **막음을 안 단다** — 이것은 고장이 아니라 실재하는 상태다
        #   (그때 앱 DB 를 못 읽어 목록이 반쪽으로 온 것 · [296]). 막음을 달면
        #   정상 동작이 매일 '★회귀' 로 올라와 아무도 안 본다([170]·[292]).
        "이름": "이 기록의 지금 버전을 못 싣고 왔다",
        "when": ["NO_VERSION:"],
        "쉬운말": "저장하려는 건이 '지금 몇 번째 판인지'를 화면이 못 받아 왔습니다.",
        "왜": "목록을 만들 때 앱 DB 를 못 읽어 **반쪽으로** 온 것일 수 있습니다.",
        "하세요": ["목록을 다시 받아 봅니다.",
                 "다시 받아도 같으면 [실행] 화면에서 앱 DB 상태를 확인합니다.",
                 "★ 새로 등록하지 마세요 — 같은 기록이 두 개가 됩니다."],
        # ★ 이 일반 갈래에는 **막음을 안 단다**([292]·[478]) — 반쪽 목록은 실재하는 상태라
        #   달면 날 때마다 '★회귀'가 되어 아무도 안 본다([170]).
        #   ⚠ 빈 문자열은 '안 단 것'이 아니다 — `t250` 은 `is None` 을 보므로
        #   `"막음": ""` 를 적으면 관문이 빨개진다(2026-08-28 실측).
    },
    {
...
def help_for(target, detail=""):
    ...
    return {
        ...
        "왜": ent["왜"],
        "하세요": list(ent["하세요"]),
        # ★ `막음` 은 **선택 칸**이다 — 정상 거절(형식·용량·빈 입력)에는
        #   막음이 없다([292] · 달면 매일 ★회귀가 되어 아무도 안 본다).
        #   그런데 여기만 대괄호라, 막음 없는 항목을 만나면 `KeyError` 로
        #   **오류 설명 자체가 500 이 됐다** — 하필 사람이 오류를 겪는 그 순간이다([169]).
        #   2026-08-28 실측: 그런 항목이 **이미 하나 있었다**
        #   (`이 기록의 지금 버전을 못 싣고 왔다`) — 내가 넷을 더하며 드러났을 뿐이다.
        # ⚠ 나머지 칸은 **필수**라 대괄호를 그대로 둔다 — `.get()` 으로
        #   바꾸면 반쪽 항목이 **빈칸으로 조용히** 나간다([172]·[169]).
        "이미막음": ent.get("막음"),
