...
BOOK = [
    ...
    },
    {
        ...
        # ★ **막음을 붙이지 않는다.** [290] 이 한 것은 **문구를 가른 것**이지
        #   403 을 안 나게 한 것이 아니다 — 담당자 화면에서 관리자 단추를 누르면
        #   이 말이 나오는 것이 **정상 동작**이다. 붙여 뒀더니 2026-08-18 실측으로
        #   그 정상 동작 10건이 매일 '★회귀'로 P1 에 올라왔다([170] — 가짜 경보가
        #   대부분이면 진짜 경보까지 아무도 안 본다). [197]·[251] 과 같은 자리다.
        #   고친날은 남긴다 — 언제 갈랐는지는 여전히 기록이다.
        # ⚠ 남은 물음은 따로다: **왜 담당자 화면이 관리자 API 를 하루 10번 부르나.**
        #   그것은 사전이 아니라 화면이 고칠 일이다(분담판 [106]).
        "막음": None, "고친날": "2026-08-16",
    },
    {
        "이름": "권한 없음",
    ...
    {
        # 2026-08-17 실측 `/api/staff/records · HTTP_ERROR:404` 4건 — 전부 8/11
        # 11:46~11:54 다. 그 길이 서버에 생긴 커밋은 **같은 날 12:12**(2e8206d)이니
        # **길이 생기기 18~26분 전**에 화면이 먼저 부른 것이다. 그 뒤로 한 번도 안 났다.
        # 즉 이 앱에서 404 는 대개 '없는 주소'가 아니라 **화면과 서버 판이 어긋난 것**
        # 이다([156] 과 같은 자리 — 고쳤는데 서버가 옛 코드).
        # ★ 막음을 붙이지 않는다. 판이 어긋나는 일은 앞으로도 있다 — 붙이면 배포할
        #   때마다 '회귀'가 되어 아무도 안 본다([170]).
        "이름": "앱과 서버 판이 어긋남",
        "when": ["HTTP_ERROR:404", "(HTTP 404"],
        "쉬운말": "화면이 찾는 자리를 서버가 아직 모릅니다. "
                "잘못 누르신 것이 아니고, 적으신 내용도 그대로 있습니다.",
        "왜": "앱을 고친 직후에는 화면이 먼저 새로워지고 서버가 잠시 뒤에 따라옵니다. "
              "그 사이에 누르면 이 말이 나옵니다.",
        "하세요": ["1분쯤 뒤 화면을 새로고침합니다(폰은 앱을 닫았다 다시 엽니다).",
                 "그래도 같으면 이 화면을 캡처해서 관리자에게 보냅니다 "
                 "— 서버를 다시 띄우면 됩니다."],
        "막음": None, "고친날": None,
    },
    {
    ...
    {
        # 2026-08-18 실측: 전국 쿠팡캠프 화면이 표·요약 둘 다 빈 채로 떴다. 원인은
        # 한 낱말 — 캠프 코드가 `esc(` 를 부르는데 이 앱의 정본은 `esc2` 다
        # (`esc` 는 두 함수 안의 **지역 const** 로만 있다). 그래서 첫 셀에서 죽었다.
        #
        # ★ 여기가 [292] 가 말한 자리다 — **★신규는 정의상 사전에 없다.** 고쳐 놓고
        #   낱말을 안 달면 이미 고친 오류가 영영 P1 로 남는다. 그러므로 [300] 을
        #   고치는 일과 이 줄을 적는 일이 **한 벌**이다.
        # ★ `막음 [300]` 의 뜻은 "화면이 안 죽는다"가 아니라 **"캠프 화면이 없는
        #   함수를 안 부른다"** 이다. 다른 이름의 `… is not defined` 까지 삼키지
        #   않는다 — 넓히면 그것들이 [288] 의 근거②(그 이름의 정의가 들어온 커밋)를
        #   못 타고 고침 근거를 잃는다.
        "이름": "화면이 없는 함수를 불렀다",
        "when": ["esc is not defined"],
        "쉬운말": "화면을 그리다 멈췄습니다. 자료를 못 받은 것이 아니고, "
                "적으신 내용이 지워진 것도 아닙니다.",
        "왜": "화면 코드가 앱에 없는 이름을 불렀습니다. 그 자리에서 그리기가 멈추므로 "
              "표와 요약이 통째로 비어 보입니다.",
        "하세요": ["화면을 새로고침합니다 — 앱이 새 코드로 바뀌었으면 그대로 풀립니다.",
                 "그래도 비면 [엑셀 저장]·[이미지 저장]은 그대로 됩니다 — "
                 "급한 보고는 그 길로 먼저 만드십니다.",
                 "이 화면을 캡처해서 관리자에게 보냅니다."],
        "막음": "[300]", "고친날": "2026-08-18",
    },
    {
        # 2026-08-18 실측: 류지영 업무센터가 13:09~13:10 에 세 번 이 오류를 냈다.
        # **코드 고장이 아니다** — index.html 인라인 스크립트 4블록 811,725자를
        # `node --check` 로 재니 문법 오류 0 이었다. 즉 **페이지가 잘려서 도착한 것**
        # 이고, 같은 시각 server_guard 가 `앱 서버 응답 지연 감지`(13:04·13:15)를
        # 적어 뒀다(그때 09:50 회차가 Z: 를 훑고 있었다).
        #
        # ★ 이 오류의 무게는 건수가 아니라 **결과**다 — 스크립트가 중간에서 끊기면
        #   앱이 **통째로 안 뜬다.** 그리고 화면이 아무 말도 못 한다(말하는 코드가
        #   그 잘린 스크립트 안에 있다). 사람에게는 그냥 반쪽짜리 화면만 남는다.
        # ★ **막음을 붙이지 않는다** — 위 끊김·404 와 같은 자리다. 서버가 늘어지면
        #   또 날 수 있고, 붙이는 순간 매일 '회귀'가 되어 아무도 안 본다([170]).
        #   진짜 고침(잘린 것을 화면이 알아채기·회차와 앱 서버 분리)은 분담판에 있다.
        "이름": "화면이 반만 내려왔다",
        "when": ["Unexpected end of input"],
        "쉬운말": "화면 파일이 중간에서 끊겨 앱이 다 뜨지 못했습니다. "
                "자료가 지워진 것이 아니고, 잘못 누르신 것도 아닙니다.",
        "왜": "앱 서버가 잠시 늘어져 화면 파일이 끝까지 오지 못했습니다. "
              "받다 만 파일이라 그 뒤 부분이 아예 실행되지 않습니다.",
        "하세요": ["화면을 새로고침합니다 — 대개 한 번이면 제대로 내려옵니다.",
                 "그래도 반쪽이면 1~2분 뒤에 다시 엽니다(회차가 도는 동안 느려집니다).",
                 "여러 번 해도 같으면 이 화면을 캡처해서 관리자에게 보냅니다."],
        "막음": None, "고친날": None,
    },
    {
        # 2026-08-17 21:17 실측 2건. **파일이 없는 것이 아니다** — 디스크에 있고
        # (webapp/brand/universal-lift-mark.png · 22,276바이트) 살아 있는 서버에
        # 물어보니 `200 image/png 22,276` 이었다. 화면이 부르는 주소도 절대경로
        # `/brand/…` 로 맞다. 즉 그때 **그 요청만 못 받은 것**이다.
        #
        # ★ 그러나 **'잠깐 끊긴 것'이라고 확언하지 않는다.** 화면은 '못 받았다'까지만
        #   알지 파일이 실제로 없는지 아닌지는 모른다([169]). 파일이 진짜 없어진 날에
        #   "잠깐 끊긴 것"이라 말하면 사람이 안 찾아본다 — 못 잡는 것보다 나쁘다([172]).
        #   그래서 **가르는 법**을 준다: 새로고침해서 보이면 끊긴 것이다.
        # ★ 막음을 붙이지 않는다 — 서버가 늘어지면 또 난다(위 끊김과 같은 자리).
        "이름": "그림 하나가 안 내려왔다",
        "when": ["img 로드 실패", "그림 로드 실패"],
        "쉬운말": "화면에 쓰는 그림 하나를 못 받았습니다. 업무 자료와는 상관이 없고, "
                "숫자나 목록이 틀어진 것도 아닙니다.",
        "왜": "그 그림만 못 받았습니다. 서버가 잠시 늘어졌을 때 그럴 수 있고, "
              "그림 파일이 실제로 없어졌을 수도 있습니다 — 둘은 화면에서 구별되지 않습니다.",
        "하세요": ["화면을 새로고침합니다.",
                 "새로고침해서 그림이 보이면 잠깐 끊긴 것입니다 — 그대로 업무를 보십니다.",
                 "여러 번 해도 그 자리가 계속 비면 파일이 없어진 것일 수 있으니 "
                 "이 화면을 캡처해서 관리자에게 보냅니다."],
        "막음": None, "고친날": None,
    },
    {
        # 2026-08-25 15:41 실측 2건. **고장이 아니다** — 이 앱은 무거운 작업을
        # 한 번에 하나만 돌린다(같은 자료를 두 번 만지거나 서로 느려지지 않게).
        # 빠져 있던 것은 **왜인지 말해 주는 것**이었다: 예전 메시지는 "다른 작업
        # 실행 중" 여섯 글자뿐이라 누른 사람이 무엇이 도는지·얼마나 기다려야
        # 하는지를 하나도 몰랐다. 이제 서버가 작업 이름과 몇 분째인지를 같이 준다.
        #
        # ★ **막음을 붙이지 않는다** — 이것은 정상 동작이라 앞으로도 난다.
        #   붙이는 순간 매일 "★회귀"로 올라와 아무도 안 본다([170]·[292] 의
        #   "관리자만 되는 일" 과 같은 자리다).
        "이름": "다른 작업이 먼저 돌고 있다",
        "when": ["다른 작업 실행 중"],
        "쉬운말": "지금 다른 일이 돌고 있어서 이 작업은 시작하지 못했습니다. "
                "잘못 누르신 것이 아니고, 자료가 상한 것도 아닙니다.",
        "왜": "이 앱은 무거운 작업을 한 번에 하나만 돌립니다. 둘이 같이 돌면 같은 "
              "자료를 두 번 만지거나 서로 느려지기 때문입니다.",
        "하세요": ["[실행] 화면에서 지금 무엇이 도는지·얼마나 됐는지 봅니다 — 거절 "
                 "문구에도 작업 이름과 몇 분째인지가 같이 나옵니다.",
                 "그 작업이 끝나면 다시 누릅니다. 회차는 대개 몇 분 안에 끝납니다.",
                 "30분이 지나도 같은 작업이 도는 중이면 이 화면을 캡처해서 "
                 "관리자에게 보냅니다(회차가 매달렸을 수 있습니다)."],
        "막음": None, "고친날": None,
    },
]
