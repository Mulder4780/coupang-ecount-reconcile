...
무엇이 막고 있었나 — 두 가지였다:
  ...
  ② 취소는 **댓글**로 온다. 접수 글은 이미 올라간 뒤라 고칠 자리가 댓글밖에 없는데,
     지금 밴드 캐시는 `comment_count` 숫자만 담고 **본문이 없다**. 그래서 이 도구는
     "댓글을 못 읽어 놓쳤을 수 있는 글이 몇 건인가"를 함께 보고한다 — 사각지대를
     0건이라 말하지 않기 위해서다. 댓글 수집은 'CSOS 리서치 및 자료 수집' 세션 몫이다.
...
def write_report(rows, hits, blind):
    ...
    with open(REPORT, "w", encoding="utf-8") as f:
        ...
        f.write(f"- 댓글을 못 읽어 **놓쳤을 수 있는 글: {blind}건**"
                " (밴드 캐시에 댓글 본문이 없음 — 수집 세션 몫)\n\n")
        if rows:
...
def main(argv=None):
    ...
    if not a.quiet:
        ...
        if blind:
            print(f"  ※ 댓글 본문이 없어 못 본 글 {blind}건 — 밴드 댓글 수집이 붙으면 함께 잡힌다")
        print(f"  {path}")
    return 0
