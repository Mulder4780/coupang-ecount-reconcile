...
def load_plan(band=None):
    ...
    try:
        ...
        # 왜 못 쟀는지를 버리지 않는다([289]) — 실패를 말하면서 이유를 안 적으면
        # 고칠 자리를 아무도 못 찾는다.
        못잰이유 = type(e).__name__
    # ★ **못 잰 것을 '정상'이라 부르지 않는다**([169]). 예전 판은 나이가 None 이어도
    #   '정상'이라 적었다 — 실측 2026-08-20: 살아 있는 서버가
    #   `나이시간: None · 상태 정상` 을 내보내고 있었다(그날의 원인은 [156] 옛 코드였지만,
    #   원인이 무엇이든 **그 한 줄 자체가 거짓**이다). 받는 쪽은 그것을 보고
    #   "이 목록은 최신이다"로 읽는다 — 낡은 목록을 최신이라 믿는 것이 제일 나쁘다.
    if 나이 is None:
        상태 = "모름"
        왜 = ("대기열 파일 시각을 못 읽었다"
              + (" (" + 못잰이유 + ")" if 못잰이유 else "")
        ...
        상태 = "낡음"
        왜 = ("5분 회차가 " + str(나이) + "시간째 대기열을 안 만들었다"
              " — 그 사이 들어온 자료가 이 목록에 안 실렸을 수 있다")
    else:
