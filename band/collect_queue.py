...
# ★ **되살아나는 일이 드문 갈래** — 화면이 "남은 건수"를 말할 때 갈라 쓰라고 둔 표다.
#   근거는 실측이다: 2026-08-25 대기열 782건 중 오염이 **661건(85%)** 이고,
#   `reports/밴드_헛수확.json` 을 세니 **10회 긁은 번호 4개 · 5회 6개가 전부 그대로**였다.
#   받아 온 본문도 `5181 → 5180 을 베낌` 처럼 이웃 글이다 — 밴드가 **안 쓴 번호**로 보인다.
#   그리고 오염 602건 중 **337개(56%)가 앞뒤 번호는 멀쩡한데 자기만 오염**인 외톨이다.
# ⚠ **그래도 "안 되살아난다"고 확언하지 않는다**([169]) — UI 가 덮은 진짜 글일 수도 있다.
#   그래서 이 표는 **거르는 데 쓰지 않는다**. 거르는 문은 `make_oneclick.wasted_nos`
#   하나뿐이고 그쪽은 번호마다 실측 근거(2회 이상)를 요구한다([425]).
#   여기서 하는 일은 **갈라 말하는 것**까지다 — 통째로 빼면 2026-08-24 처럼
