    ...
    return (function poll() {
      return sleep(POLL_MS).then(function () {
        ...
        return report(band, saved ? (timedOut ? 'partial' : 'done') : 'save-failed', {
          ...
          // ★ 실패 **이유 셈** (2026-08-28).  덤프는 흡수되면 사라지므로 이유가
          //   같이 사라진다 — 되보고 `최근` 100줄이 그것을 오래 들고 있는 유일한
          //   자리다.  낱말은 수집기가 내놓는 그 이름 그대로다([229]·[441] —
          //   어긋나면 한 건도 안 걸리면서 오류도 안 난다).
          //   갈래: no-doc(문서가 아예 안 옴) · redirect · no-time · error
          // ⚠ 이 값으로 **헛돎을 판정하지 않는다**([440]) — 가르는 것은 시간이다.
          실패이유: st.failedWhy || null,
