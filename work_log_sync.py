...
def _reason_blocker(reason: str) -> str:
    ...
    if arrived and scheduling and not waiting:
        return "확인 필요(일정 미확정)"
    # ①-a 사람이 없어서 못 간 것 (차동호 팀장 통화 2026-08-06).
    #     "자재보다 인력이 더 문제다. 서울·경기 4명으로 전국을 감당 못 해 정기점검
    #     수행 자체가 어렵다." 이것을 아래 '스케줄 문제(확인 필요·일정 미확정)' 로 묶으면
    #     **기사가 미룬 것처럼 보인다.** 기사가 미룬 것과 사람이 없는 것은 전혀 다른
    #     문제이고, 뒤쪽은 본사가 인력을 넣어야 풀린다. 그래서 따로 세어 대표가 본다.
    #     ★ 낱말이 붙어 있으리라 기대하면 안 된다. 현장은 "담당 기사 퇴사로 미배정",
    #       "용역 배정 대기" 처럼 사이에 말을 끼워 적는다(실측으로 둘 다 놓쳤다).
    if re.search(r"인력.{0,4}(부족|없|미배정)|담당.{0,6}(없|부재|퇴사|미배정)|"
                 r"기사.{0,4}(부족|없|퇴사|미배정)|배정.{0,4}(불가|대기|안|못)|"
                 r"용역.{0,6}(대기|미정|미배정|배정)|결원|충원.{0,4}(대기|필요)", t):
        return "확인 필요(인력 부족)"
    # ①-b 구매·결재 절차에 걸려 있는 것 (김준형 통화 2026-08-05).
    #     기안·승인·구매 링크 첨부를 거치느라 소모품 하나도 며칠씩 걸리고, 기사가
    #     자비로 사서 쓰는 일까지 있다. 이것을 아래 ③ '자재·부품 대기(정당)' 로 묶으면
    #     **본사 절차가 병목이라는 사실이 숫자에서 사라진다** — 현장은 이미 요청했고
    #     기다리는 쪽은 회사다. 그래서 따로 세어 대표가 볼 수 있게 한다.
    #     ★ '구매' 라는 말만으로는 안 된다. 발주가 이미 나가 물건이 오는 중인 것은
    #       정상이다(③). 결재 단계에 **멈춰 있다**는 신호가 있을 때만 가른다.
    if (re.search(r"기안|결재|품의|승인\s*(대기|지연|안|못|늦)|승인이\s*안", t)
            or re.search(r"구매(부|팀)?.{0,8}(지연|안\s*해|못\s*받|늦어|없어서|미조치)", t)
            or re.search(r"(구매|조달)\s*지연", t)):
        return "확인 필요(구매·조달 지연)"
