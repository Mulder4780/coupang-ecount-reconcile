...
# ── 기능 표 — 레고 조각 목록 (2026-08-26 형님 지시) ────────────────────
#   지시: "이 앱의 카테고링만 따로 떼서 관리할 수 있고 따로 떼서 별도의 앱을
#   만들 수 있고 다른 기능들을 넣었다 뺐다 할 수 있는 레고 같은 앱으로".
#
#   ★ **레고의 씨앗은 이미 있었다** — `SHOW_SIDE_WORK` 한 글자 스위치 ·
#     대시보드 [화면 구성] 팔레트 · `worktab-nav` 류의 CSS 클래스.
#     빠진 것은 **그 조각들을 한 곳에 적어 둔 표**였다. 근거가 CSS 클래스
#     이름과 상수에 흩어져 있으면 "무엇을 끄고 켤 수 있나"를 아무도 못 센다.
#
#   ★ **열쇠는 화면 id(`v-<열쇠>`)·메뉴 `data-v` 와 정확히 같은 글자**여야 한다.
#     어긋나면 그 조각만 조용히 안 걸리면서 오류도 안 난다(`[165]`).
#     검증이 세 곳(표·마크업·메뉴)을 매번 대 본다.
#
#   ★ **권한은 여기서 정하지 않는다**(`[162]`·`[172]`). 누가 무엇을 볼 수 있나는
#     이미 서버의 다른 자리가 판정한다 — 여기에 또 적으면 사본이 둘이 되고,
#     갈린 뒤에는 어느 쪽이 맞는지 아무도 모른다. 이 표가 정하는 것은
#     **조각의 이름·묶음·설명·못 끄는지** 넷뿐이다.
#
#   ★ `필수` 는 **끄면 되돌릴 길이 없어지는 것**이다. 대시보드를 끄면 첫 화면이
#     없고, 설정을 끄면 **다시 켤 자리가 없다** — 그 둘만 못 끈다.
FEATURE_GROUPS = ["홈", "현장 업무", "정산·매출", "보고", "관리"]
FEATURES = {
    "dash":     {"이름": "대시보드",      "묶음": "홈",        "필수": True,
                 "설명": "첫 화면 — 오늘 숫자와 할 일을 한 장으로 본다"},
    "pm":       {"이름": "정기점검",      "묶음": "현장 업무",
                 "설명": "정기점검 접수·예정·완료와 미실시 사유"},
    "as":       {"이름": "돌발 AS",       "묶음": "현장 업무",
    ...
    "settle":   {"이름": "정산",          "묶음": "정산·매출",
                 "설명": "청구·명세서·세금계산서·입금 (완료월 기준)"},
    "revenue":  {"이름": "매출 실적",     "묶음": "정산·매출",
                 "설명": "월별 매출 실적 (세금계산서 발행월 기준)"},
    "daily":    {"이름": "보고",          "묶음": "보고",
                 "설명": "대표 보고용 캡처와 일일 요약"},
    "worklog":  {"이름": "대표보고 일지", "묶음": "보고",
                 "설명": "대표 보고 일지 작성·조회"},
    "report":   {"이름": "기록",          "묶음": "보고",
                 "설명": "지난 리포트와 산출물 모아 보기"},
    "calendar": {"이름": "쿠팡 캘린더",   "묶음": "보고",
                 "설명": "정기점검·돌발 AS 일정 달력"},
    "center":   {"이름": "업무센터",      "묶음": "관리",
                 "설명": "담당자별 업무센터 바로가기"},
    "org":      {"이름": "조직도",        "묶음": "관리",
                 "설명": "사무실 배치·담당자 온오프라인"},
    "flow":     {"이름": "워크플로우",    "묶음": "관리",
                 "설명": "업무 단계 정의와 도면"},
    "sources":  {"이름": "원본 자료",     "묶음": "관리",
                 "설명": "수집된 원본 파일 목록"},
    "run":      {"이름": "실행",          "묶음": "관리",
                 "설명": "회차 실행·자동화 상태"},
    "settings": {"이름": "설정",          "묶음": "관리",  "필수": True,
                 "설명": "관리자 모드·글꼴·기능 구성"},
}


def feature_catalog():
    """레고 조각 목록을 **묶음 차례대로** 돌려준다.

    ★ 화면이 제 손으로 순서를 정하면 표를 고쳐도 안 따라온다(`[162]`).
    ★ 표에 없는 묶음이 생겨도 **조용히 빼지 않는다**(`[169]`) — 맨 뒤에 붙인다.
    """
    seen, groups = [], {}
    for key, spec in FEATURES.items():
        g = str(spec.get("묶음") or "기타")
        groups.setdefault(g, []).append({
            "열쇠": key,
            "이름": str(spec.get("이름") or key),
            "설명": str(spec.get("설명") or ""),
            "필수": bool(spec.get("필수")),
        })
    order = [g for g in FEATURE_GROUPS if g in groups]
    order += [g for g in groups if g not in order]      # 표 밖 묶음도 버리지 않는다
    for g in order:
        seen.append({"묶음": g, "조각": groups[g]})
    return {"묶음차례": order, "목록": seen,
            "필수": [k for k, v in FEATURES.items() if v.get("필수")]}
...
STAFF_CENTERS = {
    ...
    # 김형래 — 재무이사(CFO). 2026-09-02 형님 지시로 신설.
    # ★ 체크리스트에 **이 앱에 없는 일을 적지 않는다**([169]) — 적으면 그 화면을
    #   찾아 헤맨다. 아래 여섯은 전부 실재하는 화면이 답한다(매출 실적·정산·보고).
    # ★ 권한은 여기서 정하지 않는다 — 2026-08-21 형님 원칙대로 로스터에 오르면
    #   `_ALL_STAFF_ENTRY_FIELDS` 를 자동 상속한다(사람 이름으로 기능을 안 가른다).
    "kim-hyeongrae": {
        "name": "김형래", "title": "김형래 재무이사 업무센터", "badge": "재무이사",
        "checklist": [
            "월별 매출·계산서 발행 현황 확인 — 세금계산서 발행월 기준",
            "세금계산서 미발행 잔액 확인 — 근거는 ERP 진행상태(원장 빈 칸이 아니다)",
            "미청구·미수금 잔액 확인",
            "입금 대사 — 입금일·입금액·잔여 미수금",
            "실적표와 앱 집계가 다른 달의 원인 확인 — 미발행분인지 자료 밀림인지",
            "대표 보고 금액이 **무엇으로 센 숫자인지** 근거 확인",
        ],
    },
    "yoo-hyeonmin": {
...
# 조직도에 **자리가 없어도 되는 사람** (2026-08-19 지시).
# 사용자 지시: "김경원, 유수비는 둘다 대표이사라고 보면 돼 조적도에 안넣어도 돼"
#
# ★ 왜 표가 필요한가 — 흐름도(`flow_step`)는 이 둘을 **접수 경로**로 부른다
#   (실측 2026-08-19: 접수 항목 `유수비·김경원 폰`). 업무센터 자리가 필요해서가
#   아니라 **접수 창구**로 이름이 오른 것이다. 그런데 `org_watch` 는 흐름도가 부르는
#   이름이 조직도 자리에 없으면 '못 따라간 것'으로 매일 경보했다 — 자리를 만들
#   대상이 아니므로 그 경보는 **영영 안 풀린다**. 경보가 대부분 가짜면 진짜 경보가
#   묻힌다(`[170]`).
# ★ **지우는 것이 아니라 갈라 적는 것이다**(`[169]`). 여기 적힌 이름은 경보에서
#   내려가되 '자리 없음이 정상'으로 **세어서** 보고된다 — 조용히 빼면 그 사람이
#   흐름도에 있다는 사실 자체가 아무 화면에도 안 남는다.
# ★ 표에 **없는** 이름이 흐름도에 새로 생기면 **여전히 경보한다** — 그것이 이 검사의
#   값어치다. 여기에 이름을 더하는 것은 사람의 결정이지 기계가 할 일이 아니다.
NO_SEAT_BY_DESIGN = {
    "유수비": "대표이사 — 접수 경로로만 흐름도에 오른다",
    "김경원": "대표이사 — 접수 경로로만 흐름도에 오른다",
}
...
def real_works():
    ...
    spec = {
        ...
        "02_돌발AS접수": ("as", ["접수ID", "프로젝트NO", "캠프명", "접수일자", "담당기사", "진행상태",
                                ...
                                "밴드 바로가기",
                                # ── 이력·품목 (2026-09-08 지시 "언제 했는지 이력관리와
                                #    어떤 품목인지 알 수 있게 전부 표기") ──
                                #  원장에 적혀 있는데 여기서 안 담아 화면에 한 글자도 안 떴다([165]).
                                "접수시간", "등록일시", "완료예정일",
                                "문제내용", "조치내용",
                                "검증결과", "검증문제코드"]),
        "04_정기점검": ("pm", ["점검ID", "프로젝트NO", "캠프명", "점검예정일", "실제점검일", "점검상태",
                              ...
                              "ERP판매전표", "거래명세서", "담당관리자",
                              # ── 이력·품목 (2026-09-08 지시) ── 실측 v631:
                              #  점검내용 380건에 "장비 2대: 매립형 4RT 01호" 처럼
                              #  **무엇을 했고 어떤 품목인지**가 그대로 적혀 있다.
                              "점검예정시간", "점검내용", "이상내용",
                              "검증결과", "검증문제코드"]),
    }
...
def _issue_truth_rows(rows):
    ...
    if band_closed:
        ...
        out.append({
            "구분": "시스템", "ID": "BAND-DONE",
            # ★ 날짜가 없으면 `app_year_rows(kind="issue")` 가 **연도를 확인할 수
            #   없는 행**으로 보고 통째로 뺀다 - 그러면 닫았다는 사실이 어느
            #   화면에도 안 남아 조용해진다([169]).  넣는 값은 지어낸 것이 아니라
            #   **오늘 판정한 날**이다.  연도 필터 자체는 안 건드린다([172]) -
            #   넓히면 2025년 행이 새어 나온다.
            "기준일": datetime.now().strftime("%Y-%m-%d"),
            # ⚠ 제목이 `밴드 게시 미확인` 으로 **시작하면 안 된다** - 이 파일과
            #   다른 화면이 그 낱말로 세는 자리가 있어(`startswith`) **닫았다는
            #   알림 자신이 미확인 한 건으로 세인다**([165] 낱말이 겹치면
            #   오류 없이 조용히 틀린다).  실측으로 그렇게 1건이 남아 보였다.
            "문제유형": "카톡·ERP로 확인되어 닫음 - 밴드 게시 미확인",
            "담당자": "류지영", "근거상태": "완료 확인됨",
            "내용·근거": (
...
def band_collect_cutoff():
    ...
    # ★ 2026-09-04 형님 지시("밴드는 빼버려"). 밴드 수집이 멈추면 그 날짜가 **굳어**
    #   전체 최솟값이 거기 붙박인다 — 실측 09-04: 매출처업무 07-23 이 이겨 캡처가
    #   "2026-07-23 까지만 왔다"고 말했다. 카톡은 09-03 까지 왔는데도.  그러면 대표는
    #   "두 달째 아무것도 못 봤다"로 읽는다([172] 틀린 지목).
    # ★ 그때는 **카톡만** 센다 — 지금 실제로 들어오는 원천이 그것 하나다.
    #   그 칸이 **없으면**(옛 색인·합성 자료) 예전 그대로다([247]·[211]).
    if idx.get("밴드중단"):
        _kd = [d for n, d in per.items() if "카톡" in str(n) and d]
        if _kd:
            last = min(_kd)
    # ★ 원천마다 밀림이 다르다 — 합쳐 말하면 어느 쪽을 채워야 하는지 못 말한다([289]).
    # ★ `밴드별` 은 **그대로 싣는다**([169]) — 어느 원천이 어디까지 왔는지는 사실이고,
    #   조용히 빼면 밴드가 멈췄다는 것조차 아무 화면에 안 뜬다.
    out = {"읽음": bool(idx.get("읽음")), "최신": last, "오늘": today,
           "밴드별": per, "밴드중단": bool(idx.get("밴드중단")),
...
def _calendar_work_events():
    ...
    def add(when, kind, title, row, extra=None):
        ...
        e = {
            ...
            # ★ 저장할 때 쓰는 열쇠 — `원천업무ID` 와 **다른 칸**이다([172]).
            #   원장 ID 채번은 접수보다 늦어서, 이달 접수분은 `접수ID`·`점검ID` 가
            #   빈 채로 온다(실측 2026-08-25: 8월 미처리 19건 중 10건). 그러면 화면이
            #   "원장 연결이 없어 열거나 적을 수 없습니다"라 적고 **[완료로 표시]
            #   단추 자체를 안 그린다** — 담당자는 완료했는데 보고에는 미처리로 남는다.
            #   ★ 앱 DB 에는 그 행이 `_business_key`(= 프로젝트NO)로 실재하고 저장
            #     API(`_staff_store_row`)가 이미 그 키로 찾는다. 실측으로 그 13건이
            #     **전부 유일하게** 찾힌다 — 여러 건이면 서버가 거절한다(옳다).
            #   ⚠ `원천업무ID` 자체를 바꾸지 않는다 — 캘린더 중복 키·화면 짝맞추기가
            #     그 값을 쓴다. 여기서 프로젝트NO 를 넣으면 같은 프로젝트의 여러 건이
            #     한 줄로 합쳐진다([172]).
            "저장키": (str(row.get("접수ID") or row.get("점검ID") or "").strip()
                       or str(row.get("_business_key") or "").strip()
                       or str(row.get("프로젝트NO") or "").split(" · ")[0].strip()),
...
def get_orgchart():
    ...
    for i, (slug, cfg) in enumerate(STAFF_CENTERS.items()):
        ...
        mgmt.append({
            ...
            "msg": hmsg,
            "ago": hago,
            # ★ 로스터가 딱지를 말하면 그것을 쓴다. 안 말하면 예전 그대로다 —
            #   유현민 갈래를 안 건드려야 그 계약을 재는 검사가 안 깨진다([172]·[219]).
            "badge": cfg.get("badge") or ("센터장" if slug == "yoo-hyeonmin" else ""),
...
""" % build_id()).lstrip()
        ...
        if p == "/api/camps":
            ...
            try:
                ...
                # ★ 원본이 새로 왔는데 이 자료가 안 따라갔으면 **화면이 말한다**
                #   (2026-08-19 실사고 · [328]). 대표 보고를 만드는 사람이 그
                #   자리에서 봐야 뜻이 있다 — 인계 문서는 그 사람이 안 본다.
...
PAGES_PUBLISH_EVERY = 10 * 60  # 폰이 실제로 읽는 사본 — 10분(2026-08-26 지시)
#   ★ 3시간이었는데 실측으로는 **하루 77~97번** 올라가고 있었다 — 자율복구가
#     D1 실패를 재시도하며 매번 밀었기 때문이다(고장의 부작용).
#     그 재시도를 없애면(`[457]`) 3시간이 되어 **형님이 보시는 자료가 낡는다.**
#     그래서 주기를 실제 값으로 맞춘다 — 그러나 `cloud_publish` 가 **업무 내용이
#     같으면 사본을 안 써서** 커밋은 안 늘어난다(안 바뀐 회차는 커밋 0).
#   ★ D1(`--cloud`)은 폰이 **1순위로 읽는 길**이라 그대로 둔다 — `--push` 는
#     그 업로드도 같이 하므로 지금은 매 회차가 둘 다 시도한다.
