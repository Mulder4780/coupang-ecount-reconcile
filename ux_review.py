...
def main():
    ...
    #   상한을 넘는 값은 '느림'의 근거로 쓰지 않는다 — 엉뚱한 화면을 고치게 된다.
    SLOW_CAP_MS = 60_000
    slow = {}
    ...
    L = [f"# 업무센터 UX 개선 제안 (자동 분석 {time.strftime('%Y-%m-%d %H:%M')})", "",
         ...
         "- ※ **2026-08-05 이전 기록은 부풀려져 있다.** 서버가 한 번 끊길 때마다 열려 있는",
         "  탭이 스무 곳을 한꺼번에 다시 불러 같은 사건이 20건으로 적혔다(실측: 오류 2,777건",
         "  = 실제 끊김 164번). 그날 화면 쪽을 고쳐 이후 기록은 30초에 한 번만 남는다.", ""]
    if not items:
        L += ["## 지금은 눈에 띄는 문제가 없다", "",
