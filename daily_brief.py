...
def brief(day=None, data=None):
    ...
    return {
        ...
        "정기점검": {"예정": len(source_today) if (source_schedule and not use_ledger_today) else len(pm_plan),
                     ...
                     # 건바이건(장비 한 대=1건) 표시용 — 그룹으로 묶지 않는다(2026-08-12 지시)
                     "예측장비": sum(units(r) for r in source_schedule if r.get("예측점검일")),
...
def text(b):
    ...
    L.append(f"\n■ 정기점검 — {md(d, d)} 완료 {p['완료장비']}건 "
             f"(그날 예정 {p['예정장비']}건)")
    L.append(f"   {p['분기']} 진행률 {p['분기진행률']}% "
             f"(완료 {p['분기완료']}건 / 전체 예정 {p['분기예정']}건)")
    if p.get("기준일까지예정"):
        L.append(f"   기준일까지 예정 {p['기준일까지예정']}건 중 "
                 f"{p['기준일까지완료']}건 이행({p['기준일이행률']}%)")
    if p["분기예정"]:
