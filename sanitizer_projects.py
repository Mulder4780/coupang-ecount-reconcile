...
# 품목표준화 (2026-09-15 · 자재수불 세션 결과 · 경영자 선택 A안) — 정본은
# `자재수불·ERP 재고데이터 정상화 및 품목표준화 구축/.../10_REPORT/90_쿠팡소독기_품목표준화.md`.
# 대표코드는 55004 하나다. 06090 은 범용 특수제작 코드(이름 52가지)라 **합치지 않고**
# 그 안의 소독기·세척기 줄만 55004 와 같은 장비로 읽는다 — 1차분 전표는 고치지 않는다.
MAIN_CODE = "55004"
SAME_EQUIPMENT_CODES = ("55004", "06090")


def main_code(item_code, text):
    """이 거래 줄이 55004 장비인가 — 맞으면 '55004', 아니면 ''(순수 함수).
    06090 은 범용이라 **이름이 소독기·세척기일 때만** 같은 장비다([172] — 코드만 보고
    묶으면 리프트 특수제작까지 소독기로 센다)."""
    code = str(item_code or "").strip()
    if code not in SAME_EQUIPMENT_CODES:
        return ""
    if code == MAIN_CODE or kind_of(text):
        return MAIN_CODE
    return ""
...
def normalize(raw):
    ...
    for r in raw:
        ...
        rec = {
            ...
            "대표코드": main_code(_pick(c, "품목코드"), item),
...
def write_xlsx(data, path=OUT_XLSX):
    ...
    sheet("3_거래내역", ["일자", "단계", "구분", "프로젝트", "프로젝트명", "품목코드", "대표코드", "품목", "수량",
