...
def _round_verdict():
    ...
    # ★ **나를 부른 그 회차에는 양보하지 않는다**([386] · 2026-09-06 실사고).
    #   회차가 제 자신에게 양보하는 것은 어떤 경우에도 고장이다([412] 가
    #   관문에서 배운 그대로). 실측: 09:50 회차의 "업로드 투입함 원본 분류"·
    #   "다운로드 흡수" 두 단계가 매 회차 실패했는데 사유가 *"일일대조 이(가)
    #   지금 돌고 있다"* 였다 — 그 단계를 부른 것이 바로 그 일일대조다.
    #   그 양보가 조율 표에 쌓여 굶주림 경보까지 냈다(한 뿌리에서 둘이 나왔다).
    # ★ **넓히지 않는다**([172]) — 빼는 것은 나를 부른 그 하나뿐이고, 남의
    #   회차가 도는 중이면 예전 그대로 물러난다(같이 Z: 를 긁으면 둘 다 느려진다).
    # ★ **표시가 없으면 예전 그대로 양보한다**([169]) — 사람이 손으로 부른
    #   길에는 이 값이 없다. 모름을 "내 회차"로 치면 두 창이 같이 긁는다.
    내회차 = (os.environ.get("COUPANG_ROUND_JOB") or "").strip()
    if 내회차:
        도는중 = [x for x in 도는중 if str(x) != 내회차]
    if not 도는중:
        return "가능", "", ""
    주인 = str(도는중[0])
...
def guard(why="", 자원=None):
    ...
    v = check()
    if v["갈래"] == "가능":
        # ★ **통과도 자국을 남긴다.** 막을 때만 적으면 조율 표가 "8/27 이후
        #   한 번도 안 돌았다"는 **거짓 굶주림**을 매일 올린다 — 거짓 경보는
        #   진짜 경보를 덮는다([170]). [293]-12 가 정오회차·관문·원본이전에서
        #   세 번 배운 자리이고 여기가 **네 번째**다([300]).
        #   ⚠ 무인 회차는 위에서 이미 돌아갔다 — 증분 5분 회차가 convert_dump·
        #     intake 를 부르므로 여기 담으면 하루 수백 줄이 되어 40줄 상한에
        #     **사람 창 자국이 밀려난다**.
        #   ⚠ "시작 시점" 이라 적는다 — guard 는 맨 앞이라 그 일이 끝까지
        #     갔는지 **모른다**([169]). 끝냈는지는 그 스크립트의 제 자국이 말한다.
        _note("완주", "수집 문 통과 — %s(시작 시점 · 끝까지 갔는지는 그 "
                      "스크립트 자국이 말한다)" % (why or "이름 없는 일"))
        return True
    _note("양보", v["왜"], v["주인"])
    이름 = 자원 or RESOURCE
