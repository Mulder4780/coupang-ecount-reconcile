...
WANTED = {
    ...
    #   앱DB 모호 판정(band_canonical.erp_pick)이 쓰는 색인이 **ERP:sales** 인데 그 화면이
    #   등록부에도 이 목록에도 없어 **자동 몰이에서 통째로 빠져** 있었다 — 그래서 그것만
    #   사람이 손으로 받아 왔고(원본 이름 판매조회_2026_0101-0731.xlsx) 8/5 뒤가 비어
    #   모호 두 건이 안 갈렸다. 빠진 것은 실패로 안 보인다([169]).
    #   ★ 실측 2026-09-02: 로그인된 탭에서 `--find` 로 사이트맵을 읽으니 링크 824개 중
    #     **`판매조회` 가 실재**했다. 다만 그때 화면이 로그인 직후 대시보드라 **모듈 이름을
    #     못 읽었다**(선택자가 안 맞는다) — 그래서 SCREENS 에 등록하지 않았다([169]·[172]:
    #     모듈을 짐작해 넣으면 엉뚱한 화면의 Excel 을 받아 놓고 맞다고 믿게 된다).
    "sales":     {"찾을말": ["판매조회", "매출계산서조회"], "무엇": "판매조회(매출계산서조회)",
                  "kind": "ERP:sales", "프리셋": "금월(~오늘)"},
