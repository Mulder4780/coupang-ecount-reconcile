...
확인된 화면 (2026-08-07 실측)
  계정별원장                 prgId E010807 · 8/01~8/07 118행 수령 ✓ → ledger
  매출(세금)계산서현황        prgId E010845 · 8/01~8/07 수령 ✓      → tax
  회계거래현황               prgId E010847 · 8월분 수령 ✓          → slips
                             (이 화면엔 ddlSYear_DATE 가 없다 — 기간은 프리셋에서 계산할 것)
  거래처별계정별원장          prgId E010809 · 격자 자체가 없다 — 거래처 지정이 더 필요

아직 못 받은 것 (전부 재고/판매 모듈 — 모듈 전환부터)
  거래명세서현황 · 매출계산서조회 · 세금계산서현황
