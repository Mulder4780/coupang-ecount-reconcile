...
KIND_LABEL = {
    # ★ 같은 `ledger` 열쇠를 두 화면이 쓴다 — 계정별원장(E010807)과 거래처별계정별원장
    #   (E010809). 열이 다른데 흡수 규칙('적요 + 차변/대변')이 둘 다 삼킨다. 이름표까지
    #   서로 달라(여기 '계정별원장' · inbox_scan.LABEL '거래처별계정별원장') 같은 자료를
    #   두 이름으로 부르고 있었다. 이름은 맞춰 두고, **가르는 일은 아직 안 했다**
    #   (2026-08-08 — 가르려면 받는 쪽까지 같이 고쳐야 한다).
    "ERP:ledger": "계정별원장(거래처별 포함)",
    "ERP:stmt":   "거래명세서현황",
    "ERP:sales":  "매출계산서조회",
    "ERP:taxinv": "매출세금계산서",
    "ERP:tax":    "세금계산서현황",
...
WANTED = {
    # ★ 2026-09-02 형님 지시 "앱 DB가 모호한건 ERP 기준으로 전부 정리해" 에서 드러났다.
    #   앱DB 모호 판정(band_canonical.erp_pick)이 쓰는 색인이 **ERP:sales** 인데 8/5 뒤가
    #   비어 모호 두 건이 안 갈렸다 — 그래서 그 화면만 사람이 손으로 받아 왔다
    #   (원본 이름 판매조회_2026_0101-0731.xlsx). 빠진 것은 실패로 안 보인다([169]).
    #   ★ 앞선 진단이 반쪽이었다 — 그것부터 적는다([309]). 낮에 "등록부에도 이 목록에도
    #     없다"고 적고 여기에 sales 를 올렸는데, 재 보니 **등록부(config/erp_screens.json)
    #     에는 있었다.** 코드의 SCREENS 만 보고 한 말이고, load_screens() 가 등록부를
    #     덮는다는 것을 안 봤다. **없는 것과 틀린 것은 다른 사실이고 고칠 자리가 다르다.**
    #   ★ 진짜 잘못은 **등록부 값이 틀린 것**이었다(메뉴 "매출(세금)계산서조회" ·
    #     prgId E010727 · 모듈 빈칸). 사이트맵 href 실측으로 확정했다 —
    #     **판매조회 · 재고 I · E040206**(경로 "재고 I > 영업관리 > 판매").
    #     짐작이 아니다: 같은 사이트맵에서 뽑은 **대조군 셋**(계정별원장 E010807 ·
    #     매출(세금)계산서현황 E010845 · 회계거래현황 E010847)이 **코드 SCREENS 와 정확히
    #     일치**해 읽는 법이 옳음을 확인했다([272] 를 측정에 적용한 것 — 있는 것이 확실한
    #     것을 같이 물어본다). 옛 값도 실재하는 다른 화면일 수 있어 지우지 않고
    #     등록부 "메모" 칸에 남겼다([169]).
    #   ★ 그래서 sales 는 이 목록에서 **내렸다** — 찾은 화면을 "못 찾은 것"에 두면
    #     --find 가 매번 헛되이 훑고 사람은 아직 못 찾은 줄 안다([169]).
    #   ⚠ 사이트맵 <a> 의 href 에 prgId 가 그대로 있다(#...&prgId=E040206&depth=4) —
    #     **메뉴를 누르지 않고도 화면 번호를 읽는다.** 다음에 화면을 등록할 때 이 길을 쓴다
    #     ([269] 의 전면 크롬 제약 밖이다 — 전용 크롬 안 CDP 라 사람 화면을 안 뺏는다).
    "taxleft":   {"찾을말": ["잔량", "미발행", "발행잔량"], "무엇": "세금계산서 잔량",
                  "kind": "ERP:taxleft", "프리셋": "금월(~오늘)"},
    # ★ 2026-08-22 형님 지시 "erp는 전부 긁어오고 알고리즘에 반영해".
    #   그날 `survey()` 가 밀린 것 7종을 냈는데 그중 **거래명세서현황만 몰이에서
    #   통째로 빠져** 있었다(등록부에 화면이 없다). 15일 밀렸는데도 `--all` 은
    #   아무 말 없이 나머지 여섯만 돌았다 — **빠진 것은 실패로 안 보인다**([169]).
    #   메뉴 이름을 짐작해 넣지 않는다([172]) — 비슷한 이름을 누르면 엉뚱한 화면의
    #   Excel 을 받아 놓고 맞다고 믿게 된다. `--find` 가 화면에서 찾아 채운다.
    "stmt":      {"찾을말": ["거래명세서현황", "거래명세서", "명세서현황"],
                  "무엇": "거래명세서현황",
                  "kind": "ERP:stmt", "프리셋": "금월(~오늘)"},
}
