...
# ── 새 정본 자리 (2026-08-27 지시) ──────────────────────────────────────────
# 형님 지시: "`Z:\25. AI_RND_Data\2000. ULH APPS DATA\2. CSOS DATA` 이 폴더에
#            **원본데이터 폴더 번호 순서대로** 계속 저장하고 기존 자료도 이쪽으로
#            **전부 복사**해서 가져와서 관리해"
#
# ★ **아직 `ORIGIN_ROOT` 를 여기로 안 돌렸다** — 순서가 이 일의 전부다([169]).
#   이 파일의 홑 상수(`ERP_DIR`·`BAND_DIR` …)를 **직접 쓰는 곳이 194곳**이라
#   주소 한 줄이면 전부 따라오는데, **복사 전에 바꾸면 그 194곳이 빈 폴더를 보고
#   "0건"이라고 조용히 확언한다.** 이사 갈 집 주소만 먼저 알린 셈이 된다.
#   그래서 ① `data_mirror.py` 회차가 새 자리를 채우고 ② 확인된 뒤 ③ 아래
...
def unreachable_note(path):
    ...
    ⚠ 이 문구를 고치면 `autopilot._ERR_MARK`·`_RES_PATH_RE` 와 **다시 대 본다.**
      어긋나면 한 건도 안 걸리면서 오류도 안 난다([165]).  검증 [493] 이 잰다.
    """
