...
### A-3. ★ GitHub Actions 는 쓰지 않는다 (2026-08-04 시도·철회)

PC 를 안 켜고 돌리려고 `.github/workflows/auto_scout.yml` 을 넣었다가 **되돌렸다.**
합성검증이 `.github/workflows` 의 **존재 자체**를 막는다 — 절대규칙 11(쿠팡 담당자에게
어떤 메시지도 보내지 않는다)에 따라 2026-07-27 에 메일 발송 워크플로를 없앤 뒤,
어떤 워크플로도 다시 생기지 못하게 건 안전장치다. 내용이 메일과 무관해도 예외를 두지
않는다 — 한 번 열어 두면 다음에 발송 경로가 섞여 들어오기 때문이다.

**PC 가 꺼져 있을 때 쓰는 길은 기존 그대로다**: 현황 조회는 고정 주소(GitHub Pages 의
암호화 폰 사본), 입력은 CSOS 앱 → 클라우드 큐 → PC 켜지면 반영. 자동화 후보 검색은
PC 작업 스케줄러(매일 10:00)가 돈다.
