...
사용자 확정(2026-07-30, 변재선 업무를 류지영에게 이관):
- 류지영: 캠프·일정, 카톡/밴드, 현장자료, 완료일,
  거래명세서, 세금계산서, 입금, 금액 불일치 확인
...
def confirmed_owner(issue="", category="", direct=""):
    ...
    # 따라서 "PO A" 같은 시스템 대조는 계속 유현민, 입금 금액 판정은 류지영이 맡는다.
    ...
    # 변재선 업무 이관에 따라 정산·청구·입금·금액 확인도 류지영에게 묶는다.
