...
_SIGNS = (
    ...
    # 2026-09-06 실측: `RemoteDisconnected`(저쪽이 **응답 없이 끊었다**)가 70회를
    #   `모름` -> `code` 로 떨어져 조치가 사람을 **멀쩡한 코드로 보냈다**([172]·[289]).
    #   HTTP 5xx 와 **같은 뜻·같은 조치**다(기다린다 · 다시 두드리지 않는다) —
    #   갈래를 새로 만들면 이름만 둘이 되므로 **여기에 넣는다**.
    # ★ 넓히지 않는다([172]): 이 둘은 `http.client` 예외 이름과 그 메시지라 아주
    #   좁다.  회선이 끊기면 `URLError`/`gaierror` 라 **다른 낱말**이고, 그것은
    #   우리가 고칠 수 있는 쪽이라 여기 넣으면 안 된다.
    ("저쪽 서버가 안 받음", r"http 5[0-9][0-9][^0-9]|service unavailable|bad gateway"
     r"|remotedisconnected|remote end closed connection",
