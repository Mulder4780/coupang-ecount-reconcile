...
_SHARE_PROBES = [0]          # 회차당 상한 - 죽은 Z: 는 한 번이 40~156초다([443])
_SHARE_PROBE_MAX = 2


def _timeout_share_note():
    """시간초과 단계 뒤에 **그때 공유폴더가 어땠나**를 한 줄로 붙인다.

    2026-09-06 실측: `ERP·거래명세서·현장검증 확정분 큐` 가 09-05 에 600초를
    넘겼는데 09-01·09-03 은 통과했다.  곧 **매번 넘기는 것이 아니다** - 그날은
    회차 자체가 150.8분(예산 초과)이었다.  그런데 자국에는 '시간초과(600s)' 만
    남아 **코드가 느린 것인지 Z: 가 붐빈 것인지 물을 데가 없었다**([465]·[469] 가
    관문·증분에서 배운 그 자리이고 여기만 몰랐다 · [300]).

    ★ **판정하지 않는다**([162]) - 재는 것은 `source_dirs.probe_share()` 를 빌리고
      낱말은 `session_handoff.gate_share_note` 가 정한다.  여기는 사실만 적는다.
    ★ **시간초과일 때만·회차당 두 번까지**([443]) - 넓히면 이 자국이 회차를 더
      밀어낸다.  ★ **못 재면 아무 말도 안 한다**([169]) - 모름을 '멀쩡했다'로
      치면 그때부터 이 자국이 조용히 틀린 조치를 준다.
    """
    if _SHARE_PROBES[0] >= _SHARE_PROBE_MAX:
        return ""
    try:
        import source_dirs as _sd
        secs, ok = _sd.probe_share()
    except Exception:
        return ""
    _SHARE_PROBES[0] += 1
    if not ok:
        return " · 그때 공유폴더에 **닿지 못했다**(%.1f초) — 코드가 느린 것이 아닐 수 있다" % secs
    if secs >= 5.0:
        return " · 그때 공유폴더 응답 **%.1f초** — 다른 일이 같이 물고 있었다(코드가 느린 것이 아닐 수 있다)" % secs
    return ""
...
def run(name, args, timeout=600, retry=None):
    ...
    for attempt in range(retry + 1):
        ...
        if "시간초과" in str(got.get("out", "")):
            # ★ **동작은 한 톨도 안 바꾼다**([172]) - 재시도를 안 하는 것은 옳다
            #   (위 주석의 2026-08-09 실측: 회차 예산을 그 하나가 두 번 먹는다).
            #   고치는 것은 **말**이다 - "다시 해도 또 넘깁니다" 는 확언인데
            #   근거가 없다.  실측 2026-09-06: 같은 단계가 09-01·09-03 은
            #   통과하고 09-05 만 넘겼다.  그 말을 믿으면 사람은 "이 단계는
            #   원래 안 되는구나" 하고 **고칠 자리를 안 찾는다**([172]·[169]).
            got["out"] = (got["out"] + " — 이번 회차에서는 다시 안 합니다"
                          "(재시도가 회차 예산을 두 번 먹는다 · 2026-08-09 실측). "
                          "다음 회차가 이어서 합니다 — 매번 넘기는지는 그 회차가 답한다"
                          + _timeout_share_note())
...
def _run_pipeline():
    ...
    # 2.85 쿠팡 소독기·세척기 현황 (2026-09-15 지시) — ERP 엑셀·공유폴더를 읽기만 해서
    #      앱 [소독기·세척기] 화면과 보고용 엑셀을 다시 만든다. 이카운트·원장에는 안 쓴다.
    steps.append(run("소독기·세척기 현황",
                     [os.path.join(ROOT, "sanitizer_projects.py")]))
