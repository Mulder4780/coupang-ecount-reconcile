...
## 사용자 대기 항목 (2026-07-25 갱신)
1. ★ **이카운트 [IP등록]에 새 공인IP `112.154.53.212` 추가** — 회선이 유동IP라 재부팅 후 변경됨(구 220.75.164.157). 등록 전까지 ERP API 전면 불가. 유동IP 재발 방지책: 통신사 고정IP 신청 검토.
2. 등록 후 전표 시험 1건(JS-2607-041, 1,472,500원) 전송 — `python ecount_upload.py --post --limit 1`
3. 방화벽 8899 인바운드 허용(폰 접속) — netsh 규칙 아직 없음 확인됨
4. 거래처별계정별원장(쿠팡로지스틱스, 4049, 7월) 엑셀 → `inbox/` / 쿠팡 PO 목록 → `inbox/` / 카톡 내보내기 → `kakao/inbox/`
5. 밴드 앱 심사 결과(client_id/secret, ~7/31 예상)
6. 밴드 게시 누락 6건(김필우: AS-2607-001·021·028·054·055·056) — 기사 확인, 23_확인필요현황 표시됨
