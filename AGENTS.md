...
## 사용자 대기 항목
1. `--post --limit 1` Run 승인(JS-2607-041 시험 전표, 1,472,500원) — ERP 쓰기는 권한 게이트가 항상 사람 클릭을 요구함(정상)
2. 거래처별계정별원장(쿠팡로지스틱스, 4049, 해당월) 엑셀 → `inbox/` (파일명에 '원장' 포함)
3. 카톡 [대화 내보내기] .txt → `kakao/inbox/`
4. 밴드 앱 심사 결과(client_id/secret, ~7/31 예상)
