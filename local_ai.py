...
def a_todo(q):
    ...
    nxt = [x for x in (d.get("다음할일") or []) if str(x).strip()][:4]
    return {"답": ("걸린 것: " + " · ".join(bits)) if bits else "걸린 것이 없습니다.",
            "다음": ("\n".join("  " + str(x) for x in nxt)) if nxt else
                    "새 작업을 시작해도 됩니다.",
            "근거": _age_note(s), "확신": "높"}
...
def a_server(q):
    ...
    if d.get("옛코드"):
        return {"답": "★ 앱 서버가 **옛 코드**로 돌고 있습니다. 화면이 멀쩡히 숫자를 "
                ...
                "근거": _age_note(s), "확신": "높"}
    return {"답": "앱 서버는 최신 코드로 돌고 있습니다.", "다음": "",
            "근거": _age_note(s), "확신": "높"}
...
INTENTS = [
    ...
    ("앱서버", [r"(앱|서버).*(옛|낡|안 바뀌|재시작)", r"화면이 안 바뀌"], a_server),
