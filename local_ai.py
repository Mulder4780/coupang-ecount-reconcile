...
(2026-09-25 보탬: **규칙이 하나도 안 걸렸을 때만** 이 PC 안 올라마에게 한 번 물어본다
— `ollama_ai.py`. 클라우드도 로그인도 돈도 안 든다. 규칙이 답하는 길은 그대로다.
그 답은 **참고지 근거가 아니다** — 꼬리표를 달고 '클로드문구'도 그대로 같이 나간다.
⚠ 부르는 자리는 **한 곳뿐**이다. 2026-09-25 에 두 창이 각각 붙여 한 질문에 두 번
불린 적이 있다([162]) — 새로 얹기 전에 여기 이미 있는지 먼저 센다.)
답을 내는 자리에서는 언어모델을 부르지 않는다. 부를 필요가 없다. 이 업무의 질문은 몇 가지 모양으로 반복되고,
...
# ─────────────────────────────────────────────────────────────────────────────
# "어디서 확인해?" — 형님이 **다섯 번** 물으신 것 (2026-08-25 실측)
# ─────────────────────────────────────────────────────────────────────────────
# `reports/앱_자문기록.json` 실측: `미처리건은 어디서 확인해?` **3회**(8/19) ·
# `미실시건 어디서 확인해?` **2회**. 앞의 셋은 `지금할일` 로 가서 **개발자용
# 대기열 25건**(`python autopilot.py --status` …)을 받았고, 뒤의 둘은 **모름**이었다.
#
# ★ 형님이 물으신 것은 "AS·정기점검 미처리를 **어느 화면**에서 보나" 이지 개발
#   대기열이 아니다. **틀린 답은 못 하는 것보다 나쁘다**([172]) — 그래서 다섯 번
#   물으셔야 했다. 가르는 낱말은 **"어디"** 하나다: 장소를 묻는 것이다.
# ★ `지금할일` 갈래는 **한 글자도 안 건드린다**([172]·[320] 의 판단 그대로).
#   `지금 미처리된건 정리`(어디가 없다)는 예전처럼 그리로 간다.
# ★ **숫자를 지어내지 않는다**([169]). 여기서 답하는 것은 *어느 화면인가* 하나이고,
#   건수는 그 화면에 가면 메뉴 배지([432])와 상태 단추가 바로 보여 준다.
# ★ 화면 이름을 **한 곳**에 적는다([162]) — 여기저기 적으면 메뉴가 바뀐 날 이 답이
#   없는 화면을 가리킨다. 검증이 `index.html` 의 실제 메뉴와 대 본다.
WHERE_SCREENS = {
    # 화면열쇠(routeNav): (메뉴에 적힌 이름, 그 화면에서 어떻게 좁히나)
    "as":     ("돌발 AS",      "위쪽 [지연] 단추를 누르면 기준일보다 밀린 것만 남습니다"),
    "pm":     ("정기점검",     "위쪽 [지연] 단추를 누르면 기준일보다 밀린 것만 남습니다"),
    "check":  ("확인 필요",    "유형 카드를 누르면 그 유형만 펼쳐집니다"),
    "settle": ("정산",         "상태 단추로 미발행·입금대기·미청구를 갈라 봅니다"),
    "camps":  ("전국쿠팡캠프", "캠프명·담당자·전화가 한 표에 있습니다"),
    "remote": ("리모컨",       "지점·기사별 보유와 불출 이력이 있습니다"),
}

# 메뉴 옆에 **건수 배지가 붙는** 화면([432]). 파이썬과 JS 라 한 곳에 못 두므로
# 검증이 `index.html` 의 `NAV_N_WHY` 열쇠와 **대 본다** — 갈리면 이 답이
# 없는 숫자를 가리킨다([162]).
WHERE_BADGE = ("as", "pm", "check")

# 무엇을 물으면 어느 화면인가. **위에서부터** 걸리는 것을 다 모은다.
WHERE_RULES = [
    (r"돌발|A/?S", ["as"]),
    (r"정기|점검", ["pm"]),
    # 종류를 안 밝히고 "미처리·미실시"만 물으면 **둘 다** 알려 준다 —
    # 한쪽만 대면 다른 쪽 현장이 조용히 빠진다([169]).
    (r"미실시|미처리|지연|밀린|안 ?끝난|안 ?한", ["as", "pm"]),
    (r"확인.{0,2}필요|이상|문제", ["check"]),
    (r"계산서|청구|정산|미수|발행", ["settle"]),
    (r"캠프|담당자|연락처|전화", ["camps"]),
    (r"리모컨", ["remote"]),
]


def a_where(q):
    """어디서 보나 — **어느 화면인지**만 답한다.

    ★ 아무 화면도 안 걸리면 **모름**이다([169]). 아무 화면이나 대면 사람이
      엉뚱한 데를 뒤진다 — 그것이 이 갈래를 만든 이유 그 자체다.
    """
    t = q or ""
    keys = []
    for pat, ks in WHERE_RULES:
    ...
    lines = []
    for k in keys:
        label, how = WHERE_SCREENS[k]
        lines.append("  · 왼쪽 메뉴 **[%s]** — %s" % (label, how))
    # ★ 배지는 **손이 필요한 셋**에만 붙는다([432]·[172]) — 리모컨·캠프에까지
    #   "메뉴 옆 숫자" 를 말하면 그 줄이 거짓이 된다([169]). 걸린 화면에 하나라도
    #   배지가 있을 때만 적는다.
...
INTENTS = [
    ...
    # ★ **`지금할일` 바로 앞**이다. 뒤에 두면 `미처리` 가 먼저 걸려 형님이 받으셨던
    #   그 개발자 대기열이 다시 나간다. 앞의 갈래들(계산서·취소·앱서버)보다는
    #   뒤다 — 그쪽은 *장소*가 아니라 *이유*를 말해 주므로 더 값지다([292] 순서).
    ("어디서보나", [r"어디서.{0,6}(확인|보|봐|찾|볼)", r"어디.{0,4}(있|서 봐|서 확인)",
                    r"어느.{0,4}(화면|메뉴|탭|페이지)",
                    r"어떻게.{0,6}(확인|찾|봐|보)"], a_where),
...
def ask(q, log=True):
    ...
    if res:
        ...
        # ★ 그 빈자리에 **이 PC 안 동네 AI** 를 하나 얹는다 (2026-09-25 형님 지시).
        #   밖으로 한 글자도 안 나가고 크레딧도 안 쓴다. 다만 **참고지 근거가 아니다** —
        #   그래서 '클로드문구'는 지우지 않고 그대로 같이 돌려준다([169]).
        #   못 쓰면(안 떠 있음·꺼 둠·느림) 예전과 한 글자도 다르지 않다([172]).
        try:
            import ollama_ai
            if ollama_ai.ready():
                # 화면이 오래 멈추면 사람은 그냥 클로드에게 간다 — 25초에서 끊는다
                r = ollama_ai.ask(q, facts=out["클로드문구"], timeout=25.0)
                if r and r.get("답"):
                    out["동네AI"] = {"모델": r["모델"], "답": r["답"]}
                    out["답"] = ("%s(%s)의 참고 답 — 확인 필요:\n%s"
                                 % (ollama_ai.MARK, r["모델"], r["답"]))
                    out["확신"] = "참고"
                elif r and r.get("못함"):
                    out["동네AI"] = {"못함": r["못함"]}
        except Exception as e:          # 동네 AI 때문에 답변기가 죽지 않는다
            out["동네AI"] = {"못함": "%s: %s" % (type(e).__name__, e)}
    if err:
        out["오류"] = err
    if log:
        log_ask(out)
...
PROBES = [
    ...
    ("지금 뭐부터 하면 돼", "지금할일"),
    ("지금 미처리된건 정리", "지금할일"),
    # 2026-08-19 형님이 실제로 물어 헛소리를 받은 그 문장이다.
    # ★ 2026-08-25 에 갈래가 `지금할일` → `어디서보나` 로 **바뀌었다**. [320] 이
    #   고친 것은 *답의 내용*(낡은 발췌를 읊던 것)이고 그 고침은 그대로 산다.
    #   그런데 형님은 그 뒤에도 8/19 에만 세 번, 이어서 "미실시건"으로 두 번 더
    #   물으셨다 — 물으신 것은 "**어디서**", 곧 화면이었는데 앱은 개발자 대기열을
    #   내놓고 있었다. 낱말 "어디"가 그것을 가른다.
    ("미처리건은 어디서 확인해?", "어디서보나"),
    ("미실시건 어디서 확인해?", "어디서보나"),
    ("리모컨 어느 화면에서 봐", "어디서보나"),
    # ★ "어디"가 없으면 예전 그대로 `지금할일` 이다 — 갈래를 넓히지 않았다([172]).
