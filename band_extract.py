...
# * **캠프가 말한 급함** — 2026-09-09 A/S팀 협조요청문으로 접수 양식에
#   `● 긴급도 : 긴급 / 보통 / 낮음` 한 줄이 늘었다(유니웍스에 그 기능이 들어가기 전까지).
#
#   * **우리 대응등급과 다른 사실이다.** as_grade 의 `A긴급·B일반·C전화` 는
#     2026-09-09 유수비 대표 통화로 정한 **우리 AS팀의 대응**이고, 이 값은
#     **캠프 담당자가 적어 보낸 급함**이다. 02시트에도 `긴급도` 열이 따로
#     있는데 그쪽 낱말은 `보통`·`높음` 이라 또 다르다(as_grade 머리글 실측).
#     셋을 한 칸에 담으면 그 구별을 잃는다 — 그래서 **제 칸으로 담는다**
#     (형님 2026-09-09 결정: 낱말은 캠프가 쓰는 그대로 두고 앱이 따로 담는다).
#   * **낱말을 판정하지 않는다**([166]·[169]) — 양식을 안 고치고 그대로 둔
#     `긴급 / 보통 / 낮음 (유니웍스에 반영예정)` 같은 값도 지우지 않고 원문
#     그대로 담는다. 목록 밖인지는 읽는 쪽이 보고 안다.
#   * 값이 비면 안 걸린다 — 빈 칸을 급함으로 읽지 않는다.
RE_URGENCY = re.compile(r"긴급\s*도\s*[:：]?\s*([^\n●]*)")
...
def parse_post(no, p, band):
    ...
    # 캠프가 말한 급함 — 양식 머리 뒤가 주인공([172]·[376]: 한 글에 양식이
    # 여럿일 때 맨 앞 것만 읽어 완료 글이 엉뚱한 프로젝트에 붙은 전례가 있다).
    _ug = form_field(RE_URGENCY, c)
    return {"프로젝트NO": prj_no, "업무유형": kind, "비용구분": cost,
            ...
            # 캠프가 적어 보낸 급함 — **원문 그대로**다(우리 대응등급이 아니다).
            #   이 반환값을 읽는 곳이 여럿이라 기존 칸의 모양을 안 바꾼다([314]).
            "캠프긴급도": (_ug.group(1).strip() if _ug else ""),
