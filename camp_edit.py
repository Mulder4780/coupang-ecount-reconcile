...
# ★ `거래처코드근거` — 사람이 코드를 확인해 줬을 때 **누가 무엇을 근거로** 했는지.
#   2026-08-25 노승용 매니저가 `송파1_E동MB` = **CU086** 이라고 카톡으로 알려 준 것이
#   첫 사례다. 이 표는 **유니웍스에 코드를 적는 사람이 읽으므로**, 코드만 있고
#   근거가 없으면 그 값이 기계가 이름으로 맞춘 것인지 사람이 확인해 준 것인지
#   구별이 안 된다 — 그 둘은 근거의 세기가 다르다([169]).
PLAIN_KEYS = ("캠프명", "캠프주소", "거래처코드", "거래처코드근거", "비고")
...
def apply_edit(row, edit):
    ...
    if not isinstance(vals, dict):
        vals = {}
    # 기계가 이미 확정해 둔 코드(사람 값을 덮기 **전**). 아래 갈래 판정이 이것을 쓴다.
    기계코드 = str(row.get("거래처코드") or "").strip()
    origin = {}
    ...
    # ★ 사람이 코드를 확인해 줬으면 **갈래도 같이** 바뀐다([172]).
    #   안 바꾸면 화면이 코드를 보여 주면서 설명은 옛 기계 판정을 그대로 단다 —
    #   실측으로 `송파1_E동MB` 는 코드 배지 옆에 *"ERP 거래처등록에 댈 후보가 없다"*
    #   라고 적히게 된다. **한 줄이 스스로 앞뒤가 안 맞으면 아무도 안 믿는다.**
    # ★ 사람 값이 기계 확정과 **같으면 안 바꾼다**([172]). "ERP 거래처명과 정확히
    #   같다"는 기계가 확인할 수 있는 근거라 "사람이 확인해 준 값"보다 세다 —
    #   같은 값을 덮으면 그 근거를 잃는다. 바뀌는 것은 **기계가 모르거나 다르게
    #   아는 코드를 사람이 확인해 준 때**뿐이다.
    _사람코드 = str(vals.get("거래처코드") or "").strip()
    if _사람코드 and _사람코드 != 기계코드:
        row["코드갈래"] = "사람확인"
        why = str(vals.get("거래처코드근거") or "").strip()
        if not why:
            # 근거를 안 적었으면 **지어내지 않는다**([169]) — 누가 언제 적었는지만 말한다.
            누가 = str(edit.get("_고친이") or "").strip()
            때 = str(edit.get("_고친때") or "")[:10]
            why = "사람이 확인해 준 값" + (
                " (%s)" % " · ".join([x for x in (누가, 때) if x]) if (누가 or 때) else "")
        row["코드근거"] = why
        # 확정이 났으니 **추정은 지운다** — 안 지우면 엑셀 '추정 거래처코드' 칸에
        # 옛 추정이 나란히 남아 받아 본 사람이 둘 중 무엇인지 묻게 된다([169]).
        row["추정코드"] = ""
    row["사람고침"] = origin
